...
static orl_sec_handle FindSec(obj_file* ofile, char* name)
{
	...
	ORLFileScan(ofile->orl, (unsigned long)name, FindHelper);
	if (found_sec_handle == 0)
	{
...
static bool elfAddImport(arch_header* arch, libfile io)
{
	...
	arch->name = (char*)MemAlloc(_MAX_FNAME + _MAX_EXT);
...
static void peAddImport(arch_header* arch, libfile io)
{
	...
	ORLSecGetContents(export_sec, (char**)&edata);
	...
	for (i = 0; i < export_header->numNamePointer; i++)
	{
		...
		buffer = (char*)MemAlloc(sym_len + 6);
		if (coff_obj == TRUE)
		{
...
void ProcessImport(char* name)
{
	...
	arch = (arch_header*)MemAlloc(sizeof(arch_header));
	...
	switch (Options.filetype)
	{
		case WL_FTYPE_ELF:
			...
			buffer = (char*)MemAlloc(sym_len + strlen(exportedName) + 1);
