...
/**********************/
{
	...
	SortedSymbols = NULL;
	HashTable = (sym_entry**)MemAllocGlobal(HASH_SIZE * sizeof(HashTable[0]));
...
/*****************************************/
{
	...
	sfile = (sym_file*)MemAllocGlobal(sizeof(sym_file));
	sfile->first = NULL;
	sfile->next = NULL;
	sfile->arch = *arch;
	sfile->import = NULL;
...
/*****************************/
{
		...
		switch (Options.libtype)
		{
			...
			case WL_LTYPE_MLIB:
				...
				if (sfile->arch.ffname == NULL)
				{
					...
					sfile->ffname_length = strlen(sfile->arch.ffname);
					sfile->arch.name = (char*)MemAllocGlobal(_MAX_FNAME + _MAX_EXT + 1);
	...
	{
		SortedSymbols = (sym_entry**)MemAllocGlobal(NumSymbols * sizeof(SortedSymbols[0]));
	}
...
/*********************************************************************/
{
	...
	sym = (sym_entry*)MemAllocGlobal(sizeof(sym_entry) + name_len);
	sym->len = name_len;
...
/************************************/
{
	...
	for (sfile = FileTable.first; sfile != NULL; sfile = sfile->next)
	{
		...
		{
			if (prev_sfile != NULL)
			{    /* Not deleting from head of list */
				prev_sfile->next = sfile->next;

				if (FileTable.add_to == &sfile->next)
				{ /* Last node in list */
	...
	char* exportedName, processor_type processor)
{
	...
	CurrFile->import = (import_sym*)MemAllocGlobal(sizeof(import_sym));
	...
	Elf32_Sym* sym_table, processor_type processor)
{
	...
	for (i = 0; i < export_size; i++)
	{
		...
		{
			imp_sym = (elf_import_sym*)MemAllocGlobal(sizeof(elf_import_sym));
