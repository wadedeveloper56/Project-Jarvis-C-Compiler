...
/*********************/
{
	omfRec = (OmfRecord*)MemAlloc(INIT_OMF_REC_SIZE);
...
/****************************/
{
	...
	{    /* COMBINE == COMMON, record it */
		...
		cmn = (common_blk*)MemAlloc(sizeof(common_blk));
...
/*******************************/
{
	...
	{
		len = *RecPtr++;
		ln = (struct lname*)MemAlloc((sizeof(struct lname) - 1) + len);
		ln->local = local;
		ln->len = len;
...
/*********************************/
{
	if (len + 3 > omfRecLen)
	{
		OmfRecord* new1;
		omfRecLen = len + 3;
		new1 = (OmfRecord*)MemAlloc(omfRecLen);
		new1->basic.len = omfRec->basic.len;
		new1->basic.type = omfRec->basic.type;
		MemFree(omfRec);
		omfRec = new1;
	}
}
...
/*******************************************/
{
	...
	rec.time.class1 = CMT_LINKER_DIRECTIVE;
