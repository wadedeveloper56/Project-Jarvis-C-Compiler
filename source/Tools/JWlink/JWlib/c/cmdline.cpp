...
static void AddCommand(operation ops, char* name)
{
	...
	new1 = (lib_cmd*)MemAllocGlobal(sizeof(lib_cmd) + strlen(name));
	...
	new1->ops = ops;
	new1->next = *CmdListEnd;
	*CmdListEnd = new1;
	CmdListEnd = &new1->next;
}
	...
	switch (*c++)
	{
		case '-':
			...
			switch (*c)
			{
				...
				case '*':
#endif
					DO_OR_EQ(operation, ops, |=, OP_EXTRACT);
					doquotes = FALSE;
					++c;
					break;
			}
		...
		case '+':
			...
			{
				case '-':
					DO_OR_EQ(operation, ops, |=, OP_DELETE);
					doquotes = FALSE;
					++c;
					break;
				case '+':
					DO_OR_EQ(operation, ops, |=, OP_IMPORT);
					doquotes = FALSE;
...
static void ParseOneLine(char* c)
{
	...
	buff = (char*)MemAlloc(MAX_CMDLINE);
	for (;; )
	{
		eatwhite(c);
		start = c;
		switch (*c)
		{
			...
			default:
				...
				{
					AddCommand((operation)(OP_ADD | OP_DELETE), buff);
				}
				else
				{
	...
	while (notwhite(*c))
	{
		switch (my_tolower(*c))
		{
			...
			case 'r':
				...
				*mode = (operation)(OP_ADD | OP_DELETE);
				break;
			case 't':
				if (*mode != OP_NONE)
				{
