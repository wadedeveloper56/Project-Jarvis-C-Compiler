...
/*********************************/
{
	if (FmtData.type & MK_PE)
	{
		...
		FakeModule->modinfo = (module_flags)(DBI_ALL | MOD_LAST_SEG | MOD_NEED_PASS_2 | FMT_PE_XFER);
	...
	/*********************************************************/
{
	...
	if (!(LinkFlags & GOT_CHGD_FILES) || AlwaysCheckUsingDate)
	{
		...
		{
			DO_OR_EQ(file_status, list->status, |=, STAT_HAS_CHANGED);
		}
	}
	else
	{
...
/******************************/
{
	...
	{
		AddObjLib(lib->name, (lib_priority)(LIB_PRIORITY_MIN + 1));
	}
}
...
/********************************/
{
	...
	for (list = Root->files; list != NULL; list = list->next_file)
	{
		...
		else if (mod->n.next_mod != NULL)
		{
			...
			{
				DO_OR_EQ(module_flags, mod->modinfo, |=, MOD_KILL);
				mod = mod->n.next_mod;
				SetupModule(&mod, list);
			}
		}
	...
	while (mod != NULL)
	{
		DO_OR_EQ(module_flags, mod->modinfo, |=, MOD_KILL);               // no match found
		mod = mod->n.next_mod;
	}
	...
	while (mod != NULL)
	{
		...
		else if (!(mod->modinfo & MOD_VISITED))
		{
			...
			for (curr = mod->n.next_mod; curr != NULL; curr = curr->n.next_mod)
			{
				...
				{
					curr->f.source = list;
					DO_OR_EQ(module_flags, curr->modinfo, |=, MOD_VISITED);
				}
			}
			mod->f.source = list;
		}
		DO_AND_EQ(module_flags, mod->modinfo, &=, ~MOD_VISITED);
		mod = mod->n.next_mod;
	}
...
/*********************************/
{
	...
	for (sym = HeadSym; sym != NULL; sym = sym->link)
	{
		...
		{
			DO_OR_EQ(sym_info, sym->e.def->info, |=, SYM_RELOC_REFD);
		}
	}
}
...
/********************************/
{
	...
	DO_OR_EQ(sym_info, sym->info, |=, SYM_KILL);
...
/****************************************/
{
	DO_OR_EQ(module_flags, mod->modinfo, |=, ObjFormat & FMT_OBJ_FMT_MASK);
	if (CurrMod == NULL)
	{
		CurrSect->mods = mod;
	}
...
/**************************************/
{
	...
	DO_AND_EQ(module_flags, mod->modinfo, &=, ~FMT_OBJ_FMT_MASK);
	AddToModList(mod);
	ObjPass1();
}
...
/* do pass 1 on the object file */
{
	...
	if (CacheOpen(list))
	{
		...
		for (;;)
		{ /*  there may be more than 1 object module in a file */
			member = NULL;
			ignoreobj = FALSE;
			...
			else
			{
				...
				if (member != NULL)
				{
					DO_OR_EQ(module_flags, next->modinfo, |=, member->flags);
					_LnkFree(member);
				}
				if (!(list->status & STAT_HAS_MEMBER))
				{
					DO_OR_EQ(module_flags, next->modinfo, |=, list->status & DBI_MASK);
					if (list->status & STAT_LAST_SEG)
					{
						DO_OR_EQ(module_flags, next->modinfo, |=, MOD_LAST_SEG);
					}
				}
				old = CurrMod;
				AddToModList(next);
...
/*******************************************************************************/
{
	...
	*size = 0;
	...
	if (!IsORL(list, *loc))
	{
		if (IsOMF(list, *loc))
		{
			DO_OR_EQ(obj_format, ObjFormat, |=, FMT_OMF);
...
/********************************************************/
{
	...
	{
		id = (unsigned_8*)CacheRead(list, loc, sizeof(unsigned_8));
...
/* Pass 1 of 8086 linker. */
{
	...
	DO_OR_EQ(module_flags, CurrMod->modinfo, |=, MOD_DONE_PASS_1);
	SymModStart();
	...
	PermEndMod(CurrMod);
	FreeObjInfo();
	ObjFormat = (obj_format)0;       //clear flags for processing obj file
...
scan_again:
	...
	do
	{
		...
		for (lib = ObjLibFiles; lib != NULL; lib = lib->next_file)
		{
			...
			{
				DO_OR_EQ(file_status, lib->status, |=, STAT_OLD_LIB);
			}
			else
			{
				DO_OR_EQ(file_status, lib->status, |=, STAT_SEEN_LIB);
			}
		}
		for (sym = HeadSym; sym != NULL; sym = sym->link)
		{
			...
			DO_OR_EQ(sym_info, sym->info, |=, SYM_CHECKED);
		}
			...
			if (!(sym->info & (SYM_DEFINED | SYM_IS_ALTDEF)) && (sym->info & SYM_EXPORTED))
			{
				...
				if (NULL == strchr(sym->name, '@'))
				{
					...
					if (sym2)
					{
						...
						_ChkAlloc(char*, exp->impname, size + 1);
					...
					fndname = NULL;
					SetLibCmp((int (*)())fuzOMFCmp, (int (*)())fuzARCmp);
