...
/***************************************/
{
	...
	{
		DEBUG((DBG_OLD, "AutoGroupSect(): seg=%h (%s) cls=\"%s\" info=%h", seg, seg->segname, seg->class1->name, seg->info));
		if (seg->info & SEG_ABSOLUTE)
		{
			PackSegs(seg, 1);
		}
		else if (*seg->class1->name == 0)
		{ /* jwlink: added */
			PackSegs(seg, 1);
		}
		else
...
/********************************************************/
{
	...
	while (num_segs != 0)
	{
		if (seg->group == NULL || seg->group == group)
		{
			...
			if (seg->class1->flags & CLASS_COPY)
			{  // If class is copied, mark group accordingly
				group->isdup = TRUE;
			}
			if (seg->group == NULL)
			{              // if its not in a group add it to this one
				seg->group = group;
