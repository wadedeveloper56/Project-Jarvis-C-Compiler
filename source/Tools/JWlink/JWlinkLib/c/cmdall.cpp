...
/**************************/
{
	if (!GetToken(SEP_NO, (tokcontrol)(TOK_INCLUDE_DOT | TOK_IS_FILENAME)))
		return(FALSE);
	DO_AND_EQ(commandflag, CmdFlags, &=, ~CF_UNNAMED);
	if (Name != NULL)
	{
		_LnkFree(Name);
	}
...
/* add an individual alias */
{
	...
	memcpy(name, Token.this1, namelen);
	if (!GetToken(SEP_EQUALS, TOK_INCLUDE_DOT))
	{
		return(FALSE);
	}
	MakeSymAlias(name, namelen, Token.this1, Token.len);
	return(TRUE);
}
...
/******************************/
{
	...
	sym = SymOp((sym_flags)(ST_CREATE | ST_REFERENCE), Token.this1, Token.len);
	DO_OR_EQ(sym_info, sym->info, |=, SYM_DCE_REF);           /* make sure it stays around */
	return(TRUE);
}
...
/*****************************/
{
	if (GetToken(SEP_EQUALS, (tokcontrol)(TOK_INCLUDE_DOT | TOK_IS_FILENAME)) != FALSE)
	{
		if (SymFileName != NULL)
		{
			_LnkFree(SymFileName);
		}
		SymFileName = FileName(Token.this1, Token.len, E_SYM, FALSE);
...
/****************************************************/
{
	...
	_PermAlloc(file_list*, new_entry, sizeof(file_list));
	...
	if (member != NULL)
	{
		DO_OR_EQ(file_status, new_entry->status, |=, STAT_HAS_MEMBER);
	}
	return(new_entry);
}
...
/************************************************************************/
{
	...
	if (member != NULL)
	{
		_ChkAlloc(member_list*, new_member, offsetof(member_list, name) + strlen(member) + 1);
		new_member->flags = (module_flags)DBIFlag;
		...
		for (new_entry = CurrSect->files; new_entry != NULL; new_entry = new_entry->next_file)
		{
			...
			{
				DO_OR_EQ(commandflag, CmdFlags, |=, CF_MEMBER_ADDED);
	...
	if (new_member != NULL)
	{
		...
		DO_OR_EQ(infile_flags, new_entry->file->flags, |=, INSTAT_LIBRARY);
	}
	else
	{
...

{
	...
	{   /* if we need to add one */
		...
		DO_OR_EQ(infile_flags, newproc->file->flags, |=, INSTAT_LIBRARY | INSTAT_OPEN_WARNING);
...
/****************************/
{
	...
	DO_AND_EQ(commandflag, CmdFlags, &=, ~CF_MEMBER_ADDED);
	...
	*LastLibFile = entry;
	...
	if (*LastLibFile == NULL)
	{        // no file directives found yet
		CurrFList = LastLibFile;
	}
	DO_OR_EQ(infile_flags, entry->file->flags, |=, INSTAT_USE_LIBPATH);
	_LnkFree(ptr);
	return(TRUE);
}
...
/* process FILE command */
{
	...
	{
		DO_OR_EQ(commandflag, CmdFlags, |=, CF_FILES_BEFORE_DBI);
	}
	if (LastLibFile == NULL)
	{
		LastLibFile = &Root->files;
	}
	return(ProcArgList(&AddLibFile, (tokcontrol)(TOK_INCLUDE_DOT | TOK_IS_FILENAME)));
}
...
/*************************/
{
	...
	{
		if (CmdFlags & CF_SECTION_THERE)
		{     // is section there already?
			DO_AND_EQ(commandflag, CmdFlags, &=, ~CF_SECTION_THERE);
		}
		else
		{
			MakeNewSection();
...
/* process FILE command */
{
	...
	return(ProcArgList(&AddFile, (tokcontrol)(TOK_INCLUDE_DOT | TOK_IS_FILENAME)));
}

bool ProcModFiles(void)
/***************************/
{
	return(ProcArgList(&AddModFile, (tokcontrol)(TOK_INCLUDE_DOT | TOK_IS_FILENAME)));
}
...
/************************/
{
	...
	ptr = FileName(Token.this1, Token.len, E_LIBRARY, FALSE);
	result = AddObjLib(ptr, LIB_PRIORITY_MAX);
	DO_OR_EQ(file_status, result->status, |=, STAT_USER_SPECD);
	if (CmdFlags & CF_SET_SECTION)
	{
		DO_OR_EQ(file_status, result->status, |=, STAT_LIB_FIXED);
		if (OvlLevel == 0)
		{
			result->ovlref = 0;
		}
	...
	{
		DO_OR_EQ(infile_flags, result->file->flags, |=, INSTAT_NO_WARNING);
	}
...
/* process LIB command */
{
	...
	return(ProcArgList(&AddLib, (tokcontrol)(TOK_INCLUDE_DOT | TOK_IS_FILENAME)));
}
...
/****************************/
{
	...
	DO_OR_EQ(commandflag, CmdFlags, |=, CF_DOING_OPTLIB);
	retval = ProcLibrary();
	DO_AND_EQ(commandflag, CmdFlags, &=, ~CF_DOING_OPTLIB);
	return(retval);
}
...
/* process libpath command */
{
	...
	AddLibPaths(Token.this1, Token.len, TRUE);  // TRUE == add to front.
	return(TRUE);
}
...
/* process PATH option */
{
	...
	ret = GetToken(SEP_NO, (tokcontrol)(TOK_INCLUDE_DOT | TOK_IS_FILENAME));
	if (ret != FALSE)
	{
		_ChkAlloc(path_entry*, new_path, sizeof(path_entry) + Token.len);
		ptr = new_path->name;
		memcpy(ptr, Token.this1, Token.len);
		ptr[Token.len] = '\0';
		new_path->next = Path;
		Path = new_path;
...
/* process MAP option */
{
	DO_OR_EQ(mapflag, MapFlags, |=, MAP_FLAG);
	if (GetToken(SEP_EQUALS, (tokcontrol)(TOK_INCLUDE_DOT | TOK_IS_FILENAME)))
	{
		if (MapFName != NULL)
		{
			_LnkFree(MapFName);
		}
		MapFName = FileName(Token.this1, Token.len, E_MAP, FALSE);
...
/*********************************/
{
	DO_OR_EQ(commandflag, CmdFlags, |=, CF_NO_EXTENSION);
	return(TRUE);
}
...
/*****************************/
{
	return(ProcArgList(&AddDisable, (tokcontrol)0));
}
...
/*****************************/
{
	DO_OR_EQ(mapflag, MapFlags, |=, MAP_VERBOSE);
...
/******************************/
{
	DO_OR_EQ(linkflag, LinkFlags, |=, UNDEFS_ARE_OK);
...
/********************************/
{
	DO_AND_EQ(linkflag, LinkFlags, &=, ~UNDEFS_ARE_OK);
	return(TRUE);
}
...
/******************************/
{
	DO_OR_EQ(linkflag, LinkFlags, |=, REDEFS_OK);
	return(TRUE);
}
...
/******************************/
{
	DO_AND_EQ(linkflag, LinkFlags, &=, ~REDEFS_OK);
	return(TRUE);
}
...
/*********************************/
{
	...
	if (GetToken(SEP_EQUALS, (tokcontrol)(TOK_INCLUDE_DOT | TOK_IS_FILENAME)))
	{
		IncFileName = FileName(Token.this1, Token.len, E_ILK, FALSE);
	}
	else if (Name != NULL)
	{
...
/********************************/
{
	...
	else if (GetToken(SEP_NO, (tokcontrol)TOK_INCLUDE_DOT))
	{
		FmtData.resource = tostring();
	}
	else
...
/*****************************/
{
	DO_OR_EQ(mapflag, MapFlags, |=, MAP_STATICS);
	return(TRUE);
}
...
/********************************/
{
	DO_OR_EQ(mapflag, MapFlags, |=, MAP_ARTIFICIAL);
	return(TRUE);
}
...
/**********************************/
{
	DO_OR_EQ(mapflag, MapFlags, |=, MAP_ALPHA);
	return(TRUE);
}
...
/****************************/
{
	DO_OR_EQ(mapflag, MapFlags, |=, MAP_GLOBAL);
	return(TRUE);
}
...
/**************************/
{
	DO_OR_EQ(mapflag, MapFlags, |=, MAP_SORT);
...
/******************************/
{
	DO_AND_EQ(commandflag, CmdFlags, &=, ~CF_LANGUAGE_MASK);
...
/******************************/
{
	DO_OR_EQ(commandflag, CmdFlags, |=, CF_LANGUAGE_JAPANESE);
	return(TRUE);
}
...
/*****************************/
{
	DO_OR_EQ(commandflag, CmdFlags, |=, CF_LANGUAGE_CHINESE);
	return(TRUE);
}
...
/****************************/
{
	DO_OR_EQ(commandflag, CmdFlags, |=, CF_LANGUAGE_KOREAN);
	return(TRUE);
}
...
/******************************/
{
	DO_OR_EQ(linkflag, LinkFlags, |=, SHOW_DEAD);
	return(TRUE);
}
...
/*******************************/
{
	DO_OR_EQ(linkflag, LinkFlags, |=, VF_REMOVAL);
	return(TRUE);
}
...
/***************************/
{
	...
	name = (char*)malloc(Token.len + 1);
	memcpy(name, Token.this1, Token.len);
	name[Token.len] = '\0';
	SetStartSym(name);
	return(TRUE);
}
...
// force the start of a new auto-group after the previous object file.
{
	...
	else
	{
		if (CmdFlags & CF_MEMBER_ADDED)
		{
			DO_OR_EQ(module_flags, ((member_list*)LastFile)->flags, |=, MOD_LAST_SEG);
		}
		else
		{
			DO_OR_EQ(file_status, ((file_list*)LastFile)->status, |=, STAT_LAST_SEG);
		}
	}
	return(TRUE);
}
...
/****************************************************************************/
{
	...
	{
		Token.thumb = OK;
		AddBufferStringTable(&strtab, Token.this1, Token.len);
		AddCharStringTable(&strtab, ' ');
		RestoreParser();
	}
	AddCharStringTable(&strtab, '\0');
	_ChkAlloc(char*, copyptr, GetStringTableSize(&strtab));
...
/**************************/
{
	...
	if (HintFormat((exe_format)(MK_OS2 | MK_PE | MK_WIN_VXD)))
	{
		nameptr = &FmtData.u.os2.stub_file_name;
	}
	else if (HintFormat((exe_format)(MK_DOS16M)))
	{
	...
	if (!HaveEquals((tokcontrol)(TOK_INCLUDE_DOT | TOK_IS_FILENAME)))
		return(FALSE);
	name = FileName(Token.this1, Token.len, E_LOAD, FALSE);
	if (*nameptr == NULL)
	{
		*nameptr = name;
	}
...
/*****************************/
{
	...
	if (!GetToken(SEP_EQUALS, (tokcontrol)0))
	{
		return(FALSE);
	}
	FmtData.minor = 0;
	...
	if (!GetToken(SEP_PERIOD, (tokcontrol)0))
	{  /*if we don't get a minor number*/
	...
	if (!GetToken(SEP_PERIOD, (tokcontrol)0))
	{  /* if we don't get a revision*/
		return(TRUE);                 /* that's all right */
	}
	retval = getatol(&value);
	if (retval == ST_NOT_ORDINAL && Token.len == 1)
	{
		FmtData.revision = tolower(*Token.this1) - 'a' + 1;
	}
...
/****************************/
{
	...
	if (GetToken(SEP_EQUALS, (tokcontrol)(TOK_IS_FILENAME | TOK_INCLUDE_DOT)))
	{
...
/*****************************/
{
	return(ProcArgList(AddRunTime, (tokcontrol)(TOK_INCLUDE_DOT)));
}
...
/*****************************/
{
	...
	sym = SymOp((sym_flags)(ST_CREATE | ST_REFERENCE), Token.this1, Token.len);
	DO_OR_EQ(sym_info, sym->info, |=, SYM_TRACE);
	return(TRUE);
}
...
/******************************/
{
	...
	return(ProcArgList(&AddSymTrace, (tokcontrol)TOK_INCLUDE_DOT));
}
...
/*****************************/
{
	...
	_ChkAlloc(trace_info*, info, sizeof(trace_info));
	info->u.name = fname;
...
{
	LinkFlags |= TRACE_FLAG;
	return(ProcArgList(&AddModTrace, (tokcontrol)(TOK_INCLUDE_DOT | TOK_IS_FILENAME)));
}
...
/*********************************/
{
	...
	retval = getatol(&value);
	if (retval == ST_IS_ORDINAL && (value <= 0xFFFFL << FmtData.SegShift || HintFormat((exe_format)(~(MK_DOS | MK_SEGMENTED)))))
	{
...
/**********************************/
{
	...
	_ChkAlloc(ORDER_CLASS*, CurrOClass, sizeof(ORDER_CLASS));
	if (LastOClass == NULL)
	{
...
/***********************************/
{
	...
	retval = getatol(&value);
	if (retval == ST_IS_ORDINAL && (value <= 0xFFFFL || HintFormat((exe_format)(~(MK_DOS | MK_SEGMENTED)))))
	{
...
/**********************************/
{
	...
	_ChkAlloc(ORDER_SEGMENT*, CurrOSeg, sizeof(ORDER_SEGMENT));
