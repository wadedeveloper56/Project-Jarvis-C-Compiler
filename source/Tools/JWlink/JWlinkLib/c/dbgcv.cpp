...
{
	_PermAlloc(CVMODINFO*, obj->d.cv, sizeof(cvmodinfo));
...
// calculate size of the sstModule
{
	...
	{
		DBILineWalk((lineinfo*)obj->lines, CVAddLines);
	}
...
/******************************************************/
{
	cv_seginfo* info = (cv_seginfo*)_info;
...
void CVDefClass(class_entry* class1, unsigned_32 size)
/************************************************************/
{
	group_entry* group;

	if ((class1->flags & CLASS_DEBUG_INFO) == CLASS_DWARF)
		return;
	SectAddrs[CVSECT_MISC] += size;
	group = AllocGroup(AutoGrpName, &DBIGroups);
	group->g.class1 = class1;
...
/*************************************************/
{
	...
	leader->group = (group_entry*)group;
...
/*****************************/
{
	...
	{
		RingLookup(group->g.class1->segs, DefLeader, group);
		group->g.class1 = NULL;
	}
}
