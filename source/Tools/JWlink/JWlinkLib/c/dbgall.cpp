...
void DBIColClass(class_entry* class1)
/*******************************************/
{
	bool        isdbi;

	isdbi = TRUE;
	if (_stricmp(class1->name, _DwarfClass) == 0)
	{
		DO_OR_EQ(class_status, class1->flags, |=, CLASS_DWARF);
	}
	else if (_stricmp(class1->name, _MSTypeClass) == 0)
	{
		DO_OR_EQ(class_status, class1->flags, |=, CLASS_MS_TYPE);
	}
	else if (_stricmp(class1->name, _MSLocalClass) == 0)
	{
		DO_OR_EQ(class_status, class1->flags, |=, CLASS_MS_LOCAL);
	}
	else
	{
		isdbi = FALSE;
...
unsigned_16 DBIColSeg(class_entry* class1)
/************************************************/
{
	switch (class1->flags & CLASS_DEBUG_INFO)
	{
		case CLASS_DWARF:
			if (CurrMod->modinfo & DBI_TYPE)
			{
				DO_OR_EQ(module_flags, CurrMod->modinfo, |=, MOD_DBI_SEEN);
			}
	...
	/********************************************************************************/
{
	...
	if (IS_DBG_INFO(seg))
		return;
	if ((seg->class1->flags & (CLASS_STACK | CLASS_IDATA))
		&& (FmtData.dll || (FmtData.type & MK_PE)))
		return;
	prev = (segdata*)RingStep(seg->pieces, NULL);
	for (; ; )
	{
		if (prev == NULL)
			return;
		if (!prev->isdead)
			break;
		prev = (segdata*)RingStep(seg->pieces, prev);
	}
	initfn(prev, cookie);
	size = 0;
	curr = (segdata*)RingStep(seg->pieces, prev);
	while (curr != NULL)
	{
		if (!curr->isdead)
		{
		...
		curr = (segdata*)RingStep(seg->pieces, curr);
	}
	size += prev->length;
...
// called during pass 2 linnum processing
{
	if (LinkFlags & OLD_DBI_FLAG)
	{
		DBILineWalk((lineinfo*)mod->lines, ODBIGenLines);
	}
	else if (LinkFlags & DWARF_DBI_FLAG)
	{
		DBILineWalk((lineinfo*)mod->lines, DwarfGenLines);
	}
	else if (LinkFlags & CV_DBI_FLAG)
	{
		DBILineWalk((lineinfo*)mod->lines, CVGenLines);
	}
}
...
// called during pass 1 linnum processing
{
	...
	_PermAlloc(lineinfo*, info, sizeof(lineinfo) + size - 1);
	info->seg = seg;
	info->size = size;
	if (is32bit)
...
/*************************************************************/
{
	RingLookup(lines, (bool(*)(void*, void*))DoLineWalk, cbfn);
}
