...
/************************************/
{
	...
	_ChkAlloc(Elf32_Shdr*, hdr->sh32, hdr->sh_size);
...
/***********************************/
{
	...
	if (FmtData.u.elf.elf64)
	{
		...
		_ChkAlloc(Elf64_Phdr*, hdr->ph64, hdr->ph_size);
#if PHDRSECT
	...
	else
	{
		...
		_ChkAlloc(Elf32_Phdr*, hdr->ph32, hdr->ph_size);
#if PHDRSECT
...
/************************************************************************/
{
	...
	len = strlen(secname);
	name = (char*)malloc(RELA_NAME_SIZE + len);
	...
	for (group = Groups; group != NULL; group = group->next_group, currgrp++)
	{
		...
		if (relocs != NULL)
		{
			...
			else
			{
				...
				sh32->sh_addralign = 4;
				sh32->sh_size = RelocSize((RELOC_INFO*)relocs);
