...
/****************************************/
{
	...
	{
		_Pass1Alloc(comdat_info*, info, sizeof(comdat_info));
	}
	info->pieces = NULL;
	return(info);
}
...
/******************************************/
{
	...
	{
		_Pass1Alloc(comdat_piece*, piece, sizeof(comdat_piece));
	}
...
/**************************************/
{
	comdat_piece* piece = (comdat_piece*)piec;
	if (piece->free_data)
	{
		_LnkFree(piece->data);
	}
...
/****************************/
{
	...
	for (;;)
	{
		seg = (SEGDATA*)Ring2Step(segs, seg);
		if (seg == NULL) break;
...
/*************************************/
{
	...
	while (ob->curr < ob->end)
	{
		...
		ext = (extnode*)AllocNode(ExtNodes);
		ext->entry = sym;
		ext->ovlref = 0;
	}
	return;
...
/****************************/
{
	...
	sym = SymOp((sym_flags)(ST_FIND | ST_STATIC), symname->name, sym_len);
	if (sym == NULL) sym = SymOp((sym_flags)ST_FIND, symname->name, sym_len);
	if (sym == NULL)
	{
...
/***********************************/
{
	comdat_info* info = (comdat_info*)inf;
 ...
 * can't, it creates a new segment for the data */
{
	...
	if (leader == NULL)
	{
		...
		class1 = FindClass(sect, CDatClassNames[alloc], alloc > 1, alloc & 1, 0);
	}
	else
	{
		class1 = leader->class1;
...
/* this finds the length of an individual lidata block */
{
	...
	buff += sizeof(unsigned_16);
	if (count == 0)
	{  // this is followed by actual data
		repeat *= *buff;
		buff += *buff + 1;
	}
	else
	{            // it is a series of id blocks
		size = 0;
		while (count > 0)
		{
...
/* process a comdat record */
{
	...
	{    // do not support linker defined comdats
	...
	if (flags & CDAT_STATIC)
	{
		sym = SymOp((sym_flags)(ST_COMDAT | ST_STATIC | ST_FIND), symname->name, namelen);
	}
	else
	{
		sym = SymOp((sym_flags)(ST_COMDAT | ST_FIND), symname->name, namelen);
	}
	if (sym == NULL)
	{
		...
		if (sym == NULL)
		{
			...
			if (flags & CDAT_STATIC)
			{
				sym = SymOp((sym_flags)(ST_COMDAT | ST_STATIC), symname->name, namelen);
			}
			else
			{
				sym = SymOp((sym_flags)(ST_COMDAT), symname->name, namelen);
			}
		}
	}
#endif
	...
	if (!CacheIsPerm())
	{
		_ChkAlloc(unsigned_8*, piece->data, piece->length);
	...
	{
		info = (comdat_info*)RingLookup(CDatList, CompInfoSym, sym);
	...
	else
	{
		...
		info->sdata = sdata;
		info->sym = sym;
		info->flags = (sym_info)((attr & CDAT_SELECT_MASK) << CDAT_SELECT_SHIFT);
		DO_OR_EQ(sym_info, info->flags, |=, SYM_DEAD);        // assume redefined
		RingAppend(&info->pieces, piece);
#ifdef _INT_DEBUG
		...
		if (IS_SYM_COMDAT(sym))
		{
			CheckComdatSym(sym, (sym_info)(info->flags & ~SYM_DEAD));
		}
		else if (!(IS_SYM_REGULAR(sym) && sym->info & SYM_DEFINED))
		{
			...
			{    /* add it to a linker defined segment */
			...
			DO_OR_EQ(sym_info, sym->info, |=, (attr & CDAT_SELECT_MASK) << CDAT_SELECT_SHIFT);
			SetComdatSym(sym, sdata);
			DO_AND_EQ(sym_info, info->flags, &=, ~SYM_DEAD);
		}
	}
	if (info->flags & SYM_DEAD && !(LinkFlags & INC_LINK_FLAG))
	{
		DO_OR_EQ(obj_format, ObjFormat, |=, FMT_IGNORE_FIXUPP);
	}
	else
	{
		/* change OW v1.9 */
