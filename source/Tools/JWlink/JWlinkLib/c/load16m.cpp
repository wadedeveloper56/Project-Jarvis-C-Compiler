...
/******************************************************************/
{
	...
	*reloc_data = NULL;
	...
	{
		relocs = (RELOC_INFO*)Root->reloclist;
		num_relocs = Root->relocs;
		*reloc_data = (reloc_addr*)ChkLAlloc(sizeof(reloc_addr) * num_relocs);
		if (*reloc_data != NULL)
		{
...
/*********************************/
{
	...
	while (extra_sels--)
	{
		...
		if (LastSel >= 0xA000 && (LastSel & 0x1FF) == 0)
		{   // check for reserved selector
			LastSel += sizeof(gdt_info);
		}
	}
...
/*******************************************/
{
	...
	for (x = 0xA000; LastSel >= x; x += 0x200)
	{   // check for reserved selector
		LastSel += sizeof(gdt_info);
		if (LastSel > 0xFFFF)
		{
...
/**********************/
{
	...
	{
		if ((class1->flags & CLASS_DEBUG_INFO) == 0)
		{
			RingWalk(class1->segs, RemapAliasSels);
		}
	}
	/*
