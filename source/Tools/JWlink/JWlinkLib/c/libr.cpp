...
#else
int (*CmpOMFRtn)(const void*, const void*, unsigned) = (int (*)(const void*, const void*, unsigned))memcmp;
#endif
...
/****************************/
{
	...
	{
		CmpOMFRtn = (int (*)(const void*, const void*, unsigned))memcmp;
		CmpARRtn = ARCompName;
	}
	else
	{
...
#else    
		CmpOMFRtn = (int (*)(const void*, const void*, unsigned))_memicmp;
#endif
...
/****************************/
{
	CmpOMFRtn = (int (*)(const void*, const void*, unsigned))cmpOMF;
	CmpARRtn = (int (*)(const void*, const void*))cmpAR;
}

static void BadLibrary(file_list* list)
/***************************************/
{
	DO_OR_EQ(infile_flags, list->file->flags, |=, INSTAT_IOERR);
	...
	/*****************************************************************************/
{
	...
	ar_hdr = (ar_header*)CacheRead(list, *loc, sizeof(ar_header));
		...
		*loc += sizeof(ar_header);
		if (makedict && (*numdicts == 1 || *numdicts == 2))
		{
			...
			*/
			...
			dict = &list->u.dict->a;
			data = (char*)CachePermRead(list, *loc, size);
			if (*numdicts == 1)
			{
				...
				if (num > 0)
				{
					_ChkAlloc(char**, dict->fnametab, sizeof(char*) * num);
				}
			}
			else /* if ( *numdicts == 2 ) */
			{
	...
	else if (ar_hdr->name[0] == '/' && ar_hdr->name[1] == '/')
	{
		*loc += sizeof(ar_header);
		if (size)
		{
			list->strtab = (char*)CachePermRead(list, *loc, size);
		}
		*loc += MAKE_EVEN(size);
	}
	else
...
/******************************************************************************/
{
	...
	omf_hdr = (lib_header*)CacheRead(list, 0, sizeof(lib_header));
	header = (unsigned_8*)omf_hdr;
	reclength = 0;

	if (makedict)
	{
		_ChkAlloc(dict_entry*, list->u.dict, sizeof(dict_entry));
	}
	if (header[0] == 0xf0 && header[1] == 0x01)
	{
		// COFF object for PPC
	}
	else if (omf_hdr->cmd == LIB_HEADER_REC)
	{   // reading from a library
		DO_OR_EQ(file_status, list->status, |=, STAT_OMF_LIB);
	...
	{
		DO_OR_EQ(file_status, list->status, |=, STAT_AR_LIB);
		reclength = 2;
		*loc = AR_IDENT_LEN;
		numdicts = 0;
			...
			(ar_dict->num_entries > 0))
		{
			...
			_ChkAlloc(unsigned_16*, index_tab, sizeof(unsigned_16) * ar_dict->num_entries);
...
/* Search the specified library file for the specified name & make a module */
{
	...
	*/
	...
	obj->modinfo = (module_flags)(lib->status & DBI_MASK);
	return(obj);
}
...
/* allocate a chunk of dict memory, down from the top */
{
	...
	_LnkAlloc(void**, cache, sizeof(void*) * (num_buckets + 1));
	if (cache == NULL) return(NULL);
	for (bucket = 0; bucket != num_buckets; ++bucket)
	{
		_LnkAlloc(void*, cache[bucket], DIC_REC_SIZE * PAGES_IN_CACHE);
	...
	if (residue != 0)
	{
		_LnkAlloc(void*, cache[bucket], residue * DIC_REC_SIZE);
		if (cache[bucket] == NULL)
		{
			FreeDictCache(cache, bucket);
			return(NULL);
...
/* Search AR format library for specified member. */
{
	...
	dict = &lib->u.dict->a;
	result = (char**)bsearch(name, dict->fnametab, dict->num_entries, sizeof(char*), CmpARRtn);
#if 0
	if (LinkFlags & CASE_FLAG)
	{
		result = bsearch(name, dict->fnametab, dict->num_entries, sizeof(char*), ARCompName);
	}
	else
	{
		result = bsearch(name, dict->fnametab, dict->num_entries, sizeof(char*), ARCompIName);
	}
#endif
	if (result != NULL)
	{
...
/**********************************************************/
{
	...
	{
		buf = (char*)memchr(header->name, '/', AR_NAME_LEN);
		if (buf == NULL)
		{
			len = AR_NAME_LEN;
		}
