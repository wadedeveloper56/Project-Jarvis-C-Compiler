...
/************************************************/
{
	...
	for (grp = Groups; grp != NULL; grp = grp->next_group)
	{
		...
		WriteQNXRelocs((RELOC_INFO*)grp->g.grp_relocs, LMF_LINEAR_FIXUP_REC,
...
{
	seg_leader* seg = (seg_leader*)_seg;
	group_entry* grp = (group_entry*)_grp;
	unsigned_16     sflags;
	...
	{
		if (!(sflags & 1))
		{     // if can _read/_write or exec/_read
...
/*********************************/
{
	...
	WriteQNXRelocs((RELOC_INFO*)Root->reloclist, LMF_FIXUP_REC, 0);
	WriteQNXRelocs((RELOC_INFO*)FloatFixups, LMF_8087_FIXUP_REC, 0);
	record.reserved = record.spare = 0;
	...
	if (FmtData.cpu_type == 0)
	{        // no model specified, so assume...
		if (LinkState & FMT_SEEN_32_BIT)
		{
			header.cpu = 386;
		}
