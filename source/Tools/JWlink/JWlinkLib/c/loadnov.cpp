...
{
	DumpRelocList((RELOC_INFO*)Root->reloclist);
...
/********************************************************/
{
	...
	for (sym = HeadSym; sym != NULL; sym = sym->link)
	{
		...
		if (sym->p.import == DUMMY_IMPORT_PTR)
			sym->p.import = NULL;
import = (nov_import*)sym->p.import;
...
/********************************************************/
{
	...
	count = wrote = 0;
	export1 = FmtData.u.nov.exp.export1;
	while (export1 != NULL)
	{

		len = export1->len;
		sym = SymOp(ST_FIND, export1->name, len);
		if ((sym == NULL) || !(sym->info & SYM_DEFINED))
		{
			LnkMsg(WRN + MSG_EXP_SYM_NOT_FOUND, "s", export1->name);
		}
...
// Write a Novell image
{
	...
	for (group = Groups; group != NULL; group = group->next_group)
	{
		...
		if (iscode == docode)
		{  // logical XNOR would be better, but...
	...
	// add a relocation to the import record.
{
	...
	imp = (nov_import*)sym->p.import;
	if (imp == DUMMY_IMPORT_PTR)
	{
		_ChkAlloc(nov_import*, imp, sizeof(nov_import));
		sym->p.import = imp;
		imp->contents = 1;
	...
	else if (imp->contents < MAX_IMP_INTERNAL)
	{
		if (imp->contents == 2)
		{
			_ChkAlloc(nov_import*, new1, (MAX_IMP_INTERNAL - 2) * sizeof(unsigned_32) + sizeof(nov_import));
			memcpy(new1, imp, sizeof(nov_import));
			_LnkFree(imp);
			imp = new1;
			sym->p.import = imp;
		}
	...
	else
	{
		...
		if (voff == 0)
		{
			...
			{
				_ChkAlloc(nov_import*, new1, sizeof(nov_import) - sizeof(unsigned_32) +
					vblock * sizeof(unsigned_32) * 2);
				memcpy(new1, imp, sizeof(nov_import) - sizeof(unsigned_32) +
...
/**********************************/
{
	...
	if ((FmtData.u.nov.flags & DO_WATCOM_EXPORTS) && (dinfo != NULL))
	{
		...
		while (export1 != NULL)
		{
			sym = SymOp(ST_FIND, export1->name, export1->len);
			if ((sym != NULL) && !IS_SYM_IMPORTED(sym))
			{
