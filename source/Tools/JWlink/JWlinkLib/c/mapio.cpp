...
WriteFormat(24, leader->class1->name);
...
WriteFormat(23, seg->class1->name);
if (seg->group != NULL)
{
...
_ChkAlloc(seg_info* ,segs, count * sizeof(seg_info));
count = 0;
for (cl = sect->classlist; cl != NULL; cl = cl->next_class)
{
if ((cl->flags & CLASS_DEBUG_INFO) == 0)
{
seg = NULL;
while ((seg = (seg_leader*)RingStep(cl->segs, seg)) != NULL)
{
...
// NYI: completely broken for absolute segments
{
	...
	if (seg->isdead)
		return;
	leader = seg->u.leader;
	if (leader->class1->flags & CLASS_DEBUG_INFO)
		return;
	WriteFormat(16, leader->segname);
	WriteFormat(38, leader->class1->name);
	if (leader->info & SEG_ABSOLUTE)
 {
star = '*';
}
...
/***************************************************/
{
	symrecinfo* info = (symrecinfo*)_info;
...
/*********************************************************/
{
 ...
 {
_ChkAlloc(symrecinfo*, info, sizeof(symrecinfo));
info->next = NULL;
info->sym = sym;
info->mod = CurrMod;
