...
/*****************************************/
{
	...
	WriteDescriptor(0, 0, (desc_flags)(0));         // NULL GDT entry;
	WriteDescriptor(0, sizeof(TSS), (desc_flags)DR_TSS);          // TSS
	WriteDescriptor(0, sizeof(TSS), (desc_flags)(DR_BASE | DR_IS_APP)); // TSS alias
	pos = sizeof(TSS);
	size = NUM_GDT_DESCRIPTORS * sizeof(descriptor);
	WriteDescriptor(pos, size, (desc_flags)(DR_BASE | DR_IS_APP));    // GDT
	pos += size;
	size = NUM_IDT_DESCRIPTORS * sizeof(descriptor);
	WriteDescriptor(pos, size, (desc_flags)(DR_BASE | DR_IS_APP));    // IDT
	pos += size;
	size = (NumGroups + 1) * sizeof(descriptor);
	WriteDescriptor(pos, size, (desc_flags)DR_BASE);              //LDT
	WriteDescriptor(pos, size, (desc_flags)(DR_BASE | DR_IS_APP));    // LDT alias
	WriteDescriptor(0, 0, (desc_flags)(0));         // NULL IDT entry;
	WriteDescriptor(0, 0, (desc_flags)(0));         // NULL LDT entry;
	pos += size;
	for (group = Groups; group != NULL; group = group->next_group)
	{
		flags = (desc_flags)(DR_BASE | DR_IS_APP | DR_IS_USER);
		if (!(group->segflags & SEG_DATA))
		{
			DO_OR_EQ(desc_flags, flags, |=, DR_IS_CODE);
		}
