	...
	/* Process an object file. */
{
	...
	do
	{
		DO_AND_EQ(obj_format, ObjFormat, &=, ~FMT_MS_386);   // assume not a Microsoft 386 .obj file
		rec = (obj_record*)CacheRead(file, loc, sizeof(obj_record));
		if (rec == NULL)
		{
			EarlyEOF();
			break;
		...
		{
			RecNum++;
			ob.curr = (byte*)CacheRead(file, loc, len);
			if (ob.curr == NULL)
			{
				EarlyEOF();
				break;
...
/* Process an object record for pass 1 */
{
	...
	switch (cmd)
	{
		...
		case CMD_LNAMES:
			...
			break;
		case CMD_SEGD32:
			DO_AND_EQ(obj_format, ObjFormat, |=, FMT_MS_386);
		case CMD_SEGDEF:
			DO_AND_EQ(module_flags, CurrMod->modinfo, |=, MOD_NEED_PASS_2);
			ProcSegDef(ob);
			break;
		...
		case CMD_GRPDEF:
			...
			break;
		case CMD_LINN32:
			DO_OR_EQ(obj_format, ObjFormat, |=, FMT_MS_386);
		case CMD_LINNUM:
		...
		case CMD_FIXUP:         /* count the fixups for each seg_leader */
			DO_OR_EQ(module_flags, CurrMod->modinfo, |=, MOD_NEED_PASS_2);
			DoRelocs(ob);
			DO_AND_EQ(obj_format, ObjFormat, &=, ~FMT_UNSAFE_FIXUPP);
			break;
		case CMD_MODE32:
			DO_OR_EQ(obj_format, ObjFormat, |=, FMT_MS_386);
		case CMD_MODEND:
...
/******************************************************/
{
	...
	rec = (byte*)CacheRead(list, loc, sizeof(unsigned_8));
...
/**************************************************************/
{
	...
	rec = (obj_record*)CacheRead(list, *loc, sizeof(obj_record));
	if (rec == NULL)
		return(NULL);
	*loc += sizeof(obj_record);
	len = rec->length;
	name = (char*)CacheRead(list, *loc, rec->length);
	*loc += len;
	if (name == NULL)
		return(NULL);
...
/*******************************/
{
	...
	switch (directive)
	{
		...
		case LDIR_OPT_UNSAFE:
			DO_OR_EQ(obj_format, ObjFormat, |=, FMT_UNSAFE_FIXUPP);
			break;
	}
	return;
}
...
/* Process a comment record. */
{
	...
	switch (class1)
	{
		...
		case CMT_DEFAULT_LIBRARY:
			AddCommentLib((char*)(ob->curr), ob->end - ob->curr, (lib_priority)(LIB_PRIORITY_MAX - 1));
			break;
		...
		case CMT_EASY_OMF:
			...
			{
				DO_OR_EQ(obj_format, ObjFormat, |=, FMT_EASY_OMF);
			}
			break;
		...
		case 0x80:      /* Code Gen used to put bytes out in wrong order */
			if (attribute == CMT_SOURCE_NAME)
			{    /* no longer generated */
				DBIComment();
			}
			else if (attribute == CMT_LINKER_DIRECTIVE)
			{  /* linker directive */
				LinkDirective(ob);
			}
			break;
...
/* process a symbol alias directive */
{
	...
	while (ob->curr < ob->end)
	{
		...
		targetlen = *ob->curr;
		ob->curr++;
		sym = SymOp((sym_flags)(ST_FIND | ST_NOALIAS), alias, aliaslen);
...
/* Process NAMES record */
{
	...
	while (ob->curr < ob->end)
	{
		...
		entry = (list_of_names**)AllocNode(NameNodes);
...
/* process a segdef record */
{
	...
	{    /* jwlink: stack combine type added */
	...
	if (ObjFormat & FMT_EASY_OMF)
	{
		...
		if (ob->curr < ob->end)
		{      // the optional attribute field present
			if (!(*ob->curr & 0x4))
			{      // if USE32 bit not set;
				sdata->is32bit = FALSE;
			}
		}
	}
	sdata->iscode = IsCodeClass(clname->name, strlen(clname->name));
	snode = (segnode*)AllocNode(SegNodes);
	snode->entry = sdata;
...
/* Define a group. */
{
	...
	newnode = (grpnode*)AllocNode(GrpNodes);
...
/* Define symbols. */
{
	...
	while (ob->curr < ob->end)
	{
		...
		if (static_sym)
		{
			sym = SymOp((sym_flags)(ST_DEFINE_SYM | ST_STATIC), sym_name, sym_len);
		}
		else
		{
			sym = SymOp((sym_flags)ST_DEFINE_SYM, sym_name, sym_len);
		}
...
/* process a vftable reference record */
{
	...
	{
		DO_OR_EQ(sym_info, ext->entry->info, |=, SYM_VF_MARKED);
	}
}
...
/* Define all external references. */
{
	...
	if (static_sym)
	{
		flags = (sym_flags)(flags | ST_STATIC);
	}
	while (ob->curr < ob->end)
	{
		if (iscextdef)
...
/* Load object code, LEDATA and LIDATA. */
{
	...
	if (seg->isdead || seg->isabs)
	{   /* ignore dead or abs segments */
		DO_OR_EQ(obj_format, ObjFormat, |=, FMT_IGNORE_FIXUPP); /* and any corresponding fixupps */
		return;
	}
	DO_AND_EQ(obj_format, ObjFormat, &=, ~(FMT_IGNORE_FIXUPP | FMT_IS_LIDATA));
	if (lidata)
	{
		DO_OR_EQ(obj_format, ObjFormat, |=, FMT_IS_LIDATA);
	}
	if (ob->curr != ob->end)
	{
