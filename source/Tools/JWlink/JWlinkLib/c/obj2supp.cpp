...
/******************************************/
{
	save_fixup* save = (save_fixup*)_save;
	segdata* sdata;
	frame_spec  targ;
	frame_spec  frame;
	...
	{
		targ.type = (frame_type)FIX_GET_TARGET(save->flags);
		targ.u.ptr = save->target;
		frame.type = (frame_type)FIX_GET_FRAME(save->flags);
	...
	return(CalcSavedFixSize((fix_type)save->flags));
}
...
/*****************************************/
{
	save_fixup* fix = (save_fixup*)_fix;
	segdata* sdata;
	frame_type  frame;
	symbol* sym;
	...
	if (fix->flags & FIX_CHANGE_SEG)
	{
		...
		sdata = (segdata*)CarveMapIndex(CarveSegData, fix->pv_flags);
		fix->pv_flags = sdata;
		fix->flags |= FIX_CHANGE_SEG;
	}
	else
	{
		frame = (frame_type)FIX_GET_FRAME(fix->flags);
		if (FRAME_HAS_DATA(frame))
		{
			FixFrameValue(frame, (void**)(fix + 1));
		}
		frame = (frame_type)FIX_GET_TARGET(fix->flags);
		FixFrameValue(frame, &fix->target);
		if (frame == FIX_FRAME_EXT)
		{
			sym = (symbol*)fix->target;
			DO_OR_EQ(sym_info, sym->info, |=, SYM_RELOC_REFD);
			sym = UnaliasSym(ST_FIND, sym);
			DO_OR_EQ(sym_info, sym->info, |=, SYM_RELOC_REFD);
		}
	}
	return(CalcSavedFixSize((fix_type)fix->flags));
}
	...
	/*******************************************************************/
{
	...
	{
		PermSaveFixup(buff, CalcAddendSize((fix_type)save.flags));
	}
	TraceFixup((fix_type)save.flags, targ);
	if (CurrRec.data != NULL)
	{
...
/******************************************/
{
	...
	fixsize = CalcSavedFixSize((fix_type)save->flags);
	if (save->flags & FIX_CHANGE_SEG)
	{
		//sdata = (segdata *)( save->flags & ~FIX_CHANGE_SEG );
		save->flags &= ~FIX_CHANGE_SEG;
	...
	else if (LastSegData != NULL)
	{
		targ.type = (frame_type)FIX_GET_TARGET(save->flags);
		targ.u.ptr = save->target;
		datasize = CalcFixupSize((fix_type)save->flags);
		if (save->flags & FIX_ADDEND_ZERO)
		{
		...
		{
			data = (char*)save + (fixsize - CalcAddendSize((fix_type)save->flags));
...
/**********************************************************/
{
	...
	special = (fix_type)(fix->type & FIX_SPECIAL_MASK);
	...
	if (fix->imported)
	{
		...
		{  // can not get at a DLL relatively
	...
	else if (fix->type == FIX_OFFSET_24)
	{   // NYI: repair PPC damage here
...
/************************************/
{
	...
	{    // its a seg reloc and maybe an offset as well.
		...
		{
			if (isdbi && (LinkFlags & CV_DBI_FLAG))
			{    // FIXME
...
/*************************************/
{
	...
	if ((instruction == FAR_CALL_ID) || (instruction == FAR_JMP_ID))
	{
		...
		else
		{
			...
			if (fix->loc_addr.off & 0x1)
			{  // odd address, so put nop last
				...
				LastOptType = (fix_type)(fix->type & FIX_POINTER_MASK);
			}
			else
			{
...
/********************************************************/
{
	...
	ftype = (fix_type)(fix->type & (FIX_OFFSET_MASK | FIX_BASE));
	...
	else if (FmtData.type & MK_OS2_16BIT)
	{
		...
		{
			dll = (dll_sym_info*)tthread->u.sym->p.import;
			if (!dll->isordinal)
			{
