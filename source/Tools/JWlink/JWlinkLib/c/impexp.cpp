	...
	// Process the Microsoft Export keyword.
{
	...
	if (intname->len != 0)
	{
		exp->sym = SymOp((sym_flags)(ST_CREATE | ST_REFERENCE), intname->name, intname->len);
	}
	else
	{
		exp->sym = SymOp((sym_flags)(ST_CREATE | ST_REFERENCE), expname->name, expname->len);
	}
...
/****************************************/
{
	...
	dll = (dll_sym_info*)CarveAlloc(CarveDLLInfo);
	dll->isfree = FALSE;
	return dll;
}
...
/**************************************/
{
	...
	namelen = strlen(name);
	iatname = (char*)malloc(namelen + prefixlen + 1);
	memcpy(iatname, ImportSymPrefix, prefixlen);
	memcpy(iatname + prefixlen, name, namelen);
	prefixlen += namelen;
	iatname[prefixlen] = '\0';
	return(SymOp((sym_flags)(ST_CREATE), iatname, prefixlen));
}
	...
	/* process the MS import keyword definition */
{
	...
	if (!(sym->info & SYM_DEFINED))
	{
		...
		if (FmtData.type & MK_PE)
		{
			...
			DO_OR_EQ(sym_info, dll->iatsym->info, |=, SYM_DEFINED | SYM_DCE_REF);
			SET_SYM_TYPE(sym_info, dll->iatsym, SYM_IMPORTED);
...
// Read an old DLL & match ordinals of exports in it with exports in this.
{
	...
	else
	{
		...
		{
			_ChkAlloc(pe_object*, objects, head.pe.num_objects * sizeof(pe_object));
			QRead(the_file, objects, head.pe.num_objects * sizeof(pe_object), fname);
			currobj = objects;
