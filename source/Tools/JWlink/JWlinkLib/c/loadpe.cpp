...
/* _write the enumerated data pages, and set the values in the object table */
{
	...
	for (group = Groups; group != NULL; group = group->next_group)
	{
		...
		name = group->sym->name;
		leader = (seg_leader*)Ring2First(group->leaders);
	...
	/****************************************************************/
{
	...
	for (mod = PEImpList; mod != NULL; mod = mod->next)
	{
		for (imp = mod->imports; imp != NULL; imp = imp->next)
		{
			action((dll_sym_info*)imp->dll, (unsigned*)cookie);
		}
		action(NULL, (unsigned*)cookie);
	}
}
...
/*****************************************************************/
{
	...
	_ChkAlloc(entry_export**, sort, sizeof(entry_export*) * num_entries);
	...
	/**********************************************************************/
{
	...
	while (limit > 0)
	{
		pagesize = RelocSize((RELOC_INFO*)reloclist);
		if (pagesize != 0)
		{
			padme = FALSE;
			...
			DumpRelocList((RELOC_INFO*)reloclist);
			if (padme)
			{
...
/* dump the fixup table */
{
	...
	for (group = Groups; group != NULL; group = group->next_group)
	{
		reloclist = (void***)group->g.grp_relocs;
		if (reloclist != NULL)
		{
...
/*************************************/
{
	...
	len = strlen(name);
	_PermAlloc(list_of_names*, info, sizeof(list_of_names) + len);
...
/******************************************************************/
{
	...
	status = OpenResFiles((ExtraRes*)FmtData.u.pe.resources, &rinfo,
		&allopen, (ExeType)RC_TARGET_OS_WIN32, Root->outfile->fname);
...
/*****************************************************/
{
	...
	virt_mem** array = (virt_mem**)_array;
	offset      size;
	virt_mem    data;
...
/***********************************************/
{
	...
	if (leader != NULL && leader->size)
	{
		...
		_ChkAlloc(virt_mem*, sortarray, numpdatas * sizeof(virt_mem*));
		temp = sortarray;
...
/* make a PE executable file */
{
	...
	_ChkAlloc(pe_object*, object, num_objects * sizeof(pe_object));
...
#define CRC_BUFF_SIZE   (16*1024)
		_ChkAlloc(char*, buffer, CRC_BUFF_SIZE);
	...
	/***********************************************************************/
{
	...
	_ChkAlloc(unsigned_16*, ordbuf, numords * sizeof(unsigned_16));
...
/* _read a PE export table, and set ordinal values accordingly. */
{
	...
	if (nameptrsize == 0)                      /* NOTE: <-- premature return */
		return;
	_ChkAlloc(unsigned_32*, nameptrs, nameptrsize + sizeof(unsigned_32));
...
/************************************/
{
	...
	if (0 != CalcIDataSize())
	{
		...
		class1 = FindClass(Root, ConstClassName, 1, 0, 0);
		DO_OR_EQ(class_status, class1->flags, |=, CLASS_IDATA | CLASS_LXDATA_SEEN);
#else
		class1 = FindClass(Root, CoffIDataSegName, 1, 0, 0);
		class1->flags |= CLASS_IDATA | CLASS_LXDATA_SEEN;
#endif
		sdata = AllocSegData();
...
/*********************************************/
{
	...
	{
		++NumMods;
		_PermAlloc(module_import*, mod, sizeof(struct module_import));
		mod->next = PEImpList;
		PEImpList = mod;
	...
	mod->num_entries++;
	_PermAlloc(import_name*, imp, sizeof(struct import_name));
	imp->dll = sym;
	imp->imp = os2_imp;
...
{
	group_entry* group;
	seg_leader* curr = (seg_leader*)_seg;
...
/***************************/
{
	...
	for (class1 = Root->classlist; class1 != NULL; class1 = class1->next_class)
	{
		if (class1->flags & CLASS_CODE)
			code = class1;
		else if (class1->flags & CLASS_READ_ONLY)
		{
			RingWalk(class1->segs, SetConstGrp);
		}
	}
	if (code)
	{
		...
		WALK_IMPORT_SYMBOLS(sym) {
			...
			RegisterImport((dll_sym_info*)sym->p.import);
			DBIAddGlobal(sym);
		}
		...
		if (size != 0)
		{
			DO_OR_EQ(class_status, code->flags, |=, CLASS_TRANSFER);
			sdata = AllocSegData();
			sdata->length = size;
...
/**************************************/
{
	 ...
	 */
	for (; class1; class1 = class1->next_class)
	{
		if (class1->flags & CLASS_TRANSFER) break;
	}
	if (class1 == NULL)
	{
		XFerSegData = NULL;
	...
	{
		lead = (seg_leader*)RingLast(class1->segs);
		piece = (segdata*)RingLast(lead->pieces);
		CurrMod = FakeModule;
		group = lead->group;
...
/********************************************************/
{
	...
	_ChkAlloc(local_import*, imp, sizeof(local_import));
...
/*************************************/
{
	...
	DO_OR_EQ(sym_info, iatsym->info, |=, SYM_DEFINED | SYM_DCE_REF);
