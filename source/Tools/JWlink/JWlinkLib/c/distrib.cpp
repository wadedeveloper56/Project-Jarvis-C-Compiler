...
/******************************/
{
	...
	_ChkAlloc(mod_entry**, ModTable, INITIAL_MOD_ALLOC * sizeof(mod_entry*));
	CurrModHandle = 0;
	ArcBufLen = INITIAL_ARC_ALLOC;
	_ChkAlloc(arcdata*, ArcBuffer, sizeof(arcdata) + (INITIAL_ARC_ALLOC - 1) * sizeof(dist_arc));
	MakePass1Blocks();
}
 ...
 // fix this!
{
	...
	if (CurrModHandle == CurrModThere)
	{
		...
		_ChkAlloc(mod_entry**, new1, sizeof(mod_entry*) * CurrModThere * 2);
		memcpy(new1, ModTable, sizeof(mod_entry*) * CurrModThere);
		_LnkFree(ModTable);
		ModTable = new1;
		CurrModThere *= 2;
	}
...
/* set up the mod_entry arcdata field for dead code elimination */
{
	...
	{
		_PermAlloc(arcdata*, mod->x.arclist, sizeof(arcdata) - DIST_ONLY_SIZE);
	}
}
...
/***************************************/
{
		...
		&& seg->isdead)
	{
		...
		{
			seg = (segdata*)RingFirst(seg->u.leader->pieces);
			if (!seg->isdead)
			{
				return;
			}
		...
		{
			DO_OR_EQ(sym_info, sym->info, |=, SYM_DEAD);
		}
...
// postponed until now.
{
	...
	DO_OR_EQ(obj_format, ObjFormat, |=, FMT_DEBUG_COMENT);
	if ((FmtData.type & MK_OVERLAYS) && FmtData.u.dos.distribute)
	{
		_LnkFree(ArcBuffer);
		ArcBuffer = NULL;
...
/* add an arc to the arclist for the current module */
{
	...
	{
		_ChkAlloc(arcdata*, arclist, sizeof(arcdata) + (ArcBufLen * 2 - 1) * sizeof(dist_arc));
...
/* add an arc to the reference graph if it is not already in the graph */
{
	...
	if (sym->info & SYM_DEFINED)
	{
		if (sym->info & SYM_DISTRIB)
		{
			...
			if (mod->modinfo & MOD_FIXED)
			{        // add reference, as long
	...
	{   // just a reference, so it has to be added to the call graph.
		AddArc(arc);
	}
}
 ...
 * of the routines that mod references */
{
	...
	*/
	...
	if (ancestor == 0)
	{   // it's at the root, so pull it out of the graph
		arcs->numarcs = 0;
	}
}
 ...
 * position because of the position of mod */
{
	...
	DO_OR_EQ(module_flags, mod->modinfo, |=, MOD_VISITED);
	arcs = mod->x.arclist;
	ovlnum = arcs->ovlref;
	if (ovlnum != NO_ARCS_YET)
	{
		...
		while (index > 0)
		{
			...
			if (currarc.test <= MAX_NUM_MODULES)
			{     // GIANT KLUDGE!
					...
					else
					{
						if ((sym->p.seg == NULL)
							|| NewRefVector(sym, ovlnum,
								sym->p.seg->u.leader->class1->section->ovl_num))
						{
							DeleteArc(arcs, index);
						}
					}
...
// handle indirect calls and their effect on distributed libs.
{
	...
	DO_OR_EQ(module_flags, CurrMod->modinfo, |=, MOD_VISITED);        // visited
	DoRefGraph(0, MOD_DEREF(sym->u.d.modnum));
	DO_AND_EQ(module_flags, CurrMod->modinfo, &=, ~MOD_VISITED);
	arcs->ovlref = ovlsave;
	sym->u.d.ovlstate |= OVL_REF;
