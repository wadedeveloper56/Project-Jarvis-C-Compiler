...
/* _write all relocs associated with group to the file */
{
	...
	relocsize = RelocSize((RELOC_INFO*)group->g.grp_relocs);
	relocnum = relocsize / sizeof(os2_reloc_item);
	if (relocnum == 0)
		return(0);
	WriteLoad(&relocnum, 2);
	DumpRelocList((RELOC_INFO*)group->g.grp_relocs);
	return(relocsize);
}
...
/**************************************************************/
{
	...
	h = (struct dllist**)head; t = (struct dllist**)tail; i = (struct dllist*)item;
	...
	/**********************************************************/
{
	...
	_ChkAlloc(FullTypeRecord*, exe_type, sizeof(FullTypeRecord));
	...
	/********************************************************************/
{
	...
	_ChkAlloc(FullResourceRecord*, exe_res, sizeof(FullResourceRecord));
