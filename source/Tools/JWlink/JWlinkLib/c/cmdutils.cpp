	...
	else
	{
		...
		if (dir != NULL)
		{
			...
			for (;;)
			{
				...
				Token.this1 = pathin;            // dangerous?
				Token.len = strlen(pathin);
				if (!(*rtn)())
				{
					Token.this1 = NULL;
					Token.thumb = OK;   // make _sure_ we don't use Token.this1
					retval = FALSE;
					break;
				}
			}
	...
	while (entry->keyword != NULL)
	{
		...
		for (;;)
		{
			if (plen == 0 && !isupper(*key))
			{
				...
				{
					ret = (*entry->rtn)();
					DO_OR_EQ(commandflag, CmdFlags, |=, entry->flags);
				}
				else
				{
...
{
	return(ChkToString(Token.this1, Token.len));
}
...
/* parse the specified environment variable & deal with it */
{
	...
	else
	{
		...
		if (!IS_WHITESPACE(Token.next))
		{
			...
			_ChkAlloc(char*, buff, envlen + Token.len + 1);
...
/* check for a "fence", and skip it if it is there */
{
	...
	{
		return(GetToken(SEP_RCURLY, (tokcontrol)0));
	}
	return(FALSE);
}
...
/* _read input from given file */
{
	...
	buff = NULL;
	if (size < 65510)
	{       // if can alloc a chunk big enough
	...
	if (buff == NULL)
	{  // if couldn't buffer for some reason.
		_ChkAlloc(char*, buff, MAX_REC + 1); // have to have at least this much RAM
...
/******************************/
{
	...
	fname = FileName(Token.this1, Token.len, E_COMMAND, FALSE);
	file = QObjOpen(fname);
	if (file == NIL_HANDLE)
	{
		_LnkFree(fname);
...
/**********************************************************/
{
	...
	{    /* KLUDGE! we want to allow a zero*/
...
/************************************************/
{
	...
	_ChkAlloc(outfilelist*, fnode, sizeof(outfilelist));
...
/********************************************************/
{
	...
	if (*Token.this1 == '"' && namelen > 2)
	{
		namelen -= 2;
		objname = (char*)malloc(namelen);
		memcpy(objname, Token.this1 + 1, namelen);
	}
	else
	{
		objname = (char*)malloc(namelen);
		memcpy(objname, Token.this1, namelen);
	}
	if (GetToken(SEP_PAREN, TOK_INCLUDE_DOT))
	{   // got LIBNAME(LIB_MEMBER)
		fullmemb = (char*)malloc(Token.len + 1);
		memcpy(fullmemb, Token.this1, Token.len);
