...
static seg_leader* MakeNewLeader(segdata* sdata, class_entry* class1, unsigned_16 info);
static seg_leader* FindALeader(segdata* sdata, class_entry* class1, unsigned_16 info);
...
/**************************************/
{
	...
	{
		dll = (dll_sym_info*)sym->p.import;
		sym->p.import = NULL;
		DO_AND_EQ(sym_info, sym->info, &=, ~SYM_DEFINED);
...
/**************************************/
{
	...
	{
		exp = (entry_export*)sym->e.export1;
		exp->sym = sym;
		exp->impname = NULL;
		AddToExportList(exp);
	}
...
/***************************************************/
{
	comdat_piece* piece = (comdat_piece*)_piece;
	virt_mem* loc = (virt_mem*)_loc;
...
/*********************************************************/
{
	...
	{
		piece = (comdat_piece*)RingLookup(info->pieces, CheckVMemPieceDiff, &temp);
	}
	else
	{
		data = (char*)GetSegContents(sym->p.seg, temp);
		piece = (comdat_piece*)RingLookup(info->pieces, CheckMemPieceDiff, &data);
	}
	if (piece == NULL)
	{       // found a match
...
/************************************************/
{
	comdat_info* info = (comdat_info*)_info;
...
/*********************************************/
{
	...
	DO_OR_EQ(sym_info, info->sym->info, |=, info->flags & SYM_CDAT_SEL_MASK);
	if (!CheckSameData(mainsym, info))
	{
		mainsym = (symbol*)RingLookup(mainsym->u.altdefs, CheckAltSym, info);
		if (mainsym == NULL)
		{
			StoreInfoData(info);
			DO_OR_EQ(sym_info, info->sym->info, |=, SYM_HAS_DATA);
		}
	}
}
...
/***********************************/
{
	...
	{
		flags = (sym_flags)(ST_CREATE | ST_REFERENCE);
		if (sym->info & SYM_STATIC)
		{
			DO_OR_EQ(sym_flags, flags, |=, ST_STATIC);
		}
		...
		else if (IS_SYM_COMDAT(sym))
		{
			...
			DefineComdat(sym->p.seg, mainsym, sym->addr.off,
				(sym_info)(sym->info & SYM_CDAT_SEL_MASK), (unsigned_8*)data);
		}
	...
	else
	{
		...
		else if (IS_SYM_COMDAT(sym))
		{
			...
			DefineComdat(sym->p.seg, sym, sym->addr.off,
				(sym_info)(sym->info & SYM_CDAT_SEL_MASK),
				(unsigned_8*)GetSegContents(sym->p.seg, sym->p.seg->data));
		}
...
// NYI:  this doesn't take into account 16 & 32 bit classes with the same name.
{
	...
	for (class1 = Root->classlist; class1 != NULL; class1 = class1->next_class)
	{
		if (_stricmp(class1->name, name) == 0)
		{
			return(class1);
		}
	}
...
/************************************************/
{
	return(_stricmp(((seg_leader*)leader)->segname, (char*)name) == 0);
}

static bool DefIncGroup(void* _def, void* _grouptab)
/****************************************************/
{
	incgroupdef* def = (incgroupdef*)_def;
	group_entry*** grouptab = (group_entry***)_grouptab;
	group_entry* group;
	unsigned        index;
	char** currname;
	class_entry* class1;
	...
	{
		class1 = FindNamedClass(*currname);
		currname++;
		if (class1 != NULL)
		{
			leader = (seg_leader*)RingLookup(class1->segs, CmpSegName, *currname);
			if (leader != NULL)
			{
...
/********************************/
{
	...
	_ChkAlloc(group_entry**, IncGroups, sizeof(group_entry*) * numgroups);
	grouptab = IncGroups;
...
// make sure that the executable format is a 386 format.
{
	...
	if (!HintFormat((exe_format)MK_ALLOW_32))
	{
		if (!(ObjFormat & FMT_TOLD_XXBIT))
		{
			DO_OR_EQ(obj_format, ObjFormat, |=, FMT_TOLD_XXBIT);
#if 0 /* JWLink */
			LnkMsg(WRN + MSG_FOUND_XXBIT_OBJ, "sd", CurrMod->f.source->file->name, 32);
#endif
		}
	}
}
...
/******************************/
{
	...
	if (!HintFormat((exe_format)MK_ALLOW_64))
	{
		if (!(ObjFormat & FMT_TOLD_XXBIT))
		{
			DO_OR_EQ(obj_format, ObjFormat, |=, FMT_TOLD_XXBIT);
			LnkMsg(WRN + MSG_FOUND_XXBIT_OBJ, "sd", CurrMod->f.source->file->name, 64);
		}
	}
}
...
/******************************/
{
	if (!HintFormat((exe_format)MK_ALLOW_16))
	{
		if (!(ObjFormat & FMT_TOLD_XXBIT))
		{
			DO_OR_EQ(obj_format, ObjFormat, |=, FMT_TOLD_XXBIT);
			LnkMsg(WRN + MSG_FOUND_XXBIT_OBJ, "sd", CurrMod->f.source->file->name, 16);
		}
	}
}
...
/***********************************************************/
{
	...
	class1 = FindClass(sect, clname, sdata->is32bit, sdata->iscode, sdata->combine);
	...
	if (FmtData.type & MK_PE)
	{
		...
		for (flag_list = (seg_flags*)FmtData.u.os2.os2_seg_flags; flag_list != NULL; flag_list = flag_list->next)
		{
			if (flag_list->type == SEGFLAG_SEGMENT && 0 == strcmp(flag_list->name, sdata->u.name))
			{
				...
				if (sdata->isreadonly && !(class1->flags & CLASS_READ_ONLY))
				{
					class1 = FindClass(sect, ConstClassName, sdata->is32bit, sdata->iscode, sdata->combine);
					DO_OR_EQ(class_status, class1->flags, |=, CLASS_READ_ONLY);
				}
				break;
			}
		}
...
void AddSegment(segdata* sd, class_entry* class1)
/*******************************************************/
/* Add a segment to the segment list for an object file */
{
	...
	DEBUG((DBG_OLD, "AddSegment(seg=%s, cls=\"%s\"): size=%h comb=%x alignment=%x code=%x readonly=%x is32=%x class.flags=%x [code=%x readonly=%x 32bit=%x]",
		sd->u.name, class1->name,
		sd->length, sd->combine, sd->align, sd->iscode, sd->isreadonly, sd->is32bit,
		class1->flags,
		class1->flags & CLASS_CODE ? 1 : 0,
		class1->flags & CLASS_READ_ONLY ? 1 : 0,
		class1->flags & CLASS_32BIT ? 1 : 0));
	info = 0;
	if (sd->is32bit)
	{
		info |= USE_32;
	}
	if (class1->flags & CLASS_CODE)
	{
		info |= SEG_CODE;
	}
	 ...
	 */
	 //if( FmtData.type & MK_PE && class1->flags & CLASS_READ_ONLY ) {
	 //    info |= SEG_READ_ONLY;
	 //}
	...
	{
		leader = MakeNewLeader(sd, class1, info);
	}
	else
	{
		char* seg_name = sd->u.name;

		leader = FindALeader(sd, class1, info);
	...
	/*********************************************************************/
{
	...
	if (iscode)
	{
		DO_OR_EQ(class_status, currclass->flags, |=, CLASS_CODE);
	}
	if (IsConstClass(name, namelen))
	{
		DO_OR_EQ(class_status, currclass->flags, |=, CLASS_READ_ONLY);
	}
	if (comb == COMBINE_STACK) /* jwlink: */
		DO_OR_EQ(class_status, currclass->flags, |=, CLASS_STACK);
	else
...
static void CheckForLast(seg_leader* seg, class_entry* class1)
/*************************************************************/
// check to see if this segment should be the last one in an autogroup.
{
	if (CurrMod->modinfo & MOD_LAST_SEG && class1->flags & CLASS_CODE)
	{
		if (LastCodeSeg != NULL)
		{             // more than one code seg
...
/****************************************************/
{
	seg_leader* seg = (seg_leader*)_seg;
	segdata* sdata = (segdata*)_sdata;
...
/********************************************************/
{
	...
	{
		first = (segdata*)RingFirst(seg->pieces);
...
/************************************************************************************/
{
	...
	leader = (seg_leader*)RingLookup(class1->segs, CheckClassName, sdata);
	if (leader == NULL)
	{
		leader = MakeNewLeader(sdata, class1, info);
	}
	else
	{
...
/*************************************/
{
	...
	seg = (seg_leader*)CarveAlloc(CarveLeader);
	seg->next_seg = NULL;
	seg->grp_next = NULL;
	seg->pieces = NULL;
	seg->class1 = NULL;
...
/**************************************************************************************/
{
	...
	DEBUG((DBG_OLD, "objpass1.MakeNewLeader(%s, cls=\"%s\", flags=%x) seg->isreadonly=%x class->flags=%x", sdata->u.name, class1->name, info, sdata->isreadonly ? 1 : 0, class1->flags));
	...
	leader->class1 = class1;
	leader->info = info;
	leader->dbgtype = DBIColSeg(class1);
	CheckForLast(leader, class1);
	RingAppend(&class1->segs, leader);
...
{
	sym->mod = mod;
	SET_SYM_TYPE(sym_info, sym, type);
	...
	// do the object file independent public symbol definition.
{
	...
	else
	{
		...
		if (IS_SYM_IMPORTED(sym))
		{
			...
			{
				dll_sym_info* dll_info = (dll_sym_info*)sym->p.import;
				AddPEImportLocalSym(sym, dll_info->iatsym);
				DO_OR_EQ(sym_info, sym_type, |=, SYM_REFERENCED);
		...
		DO_AND_EQ(sym_info, sym->info, &=, ~SYM_DISTRIB);
		if (seg != NULL)
		{
			...
			if (seg->info & SEG_CODE)
			{
				...
				{
					DO_OR_EQ(sym_info, sym->info, |=, SYM_DISTRIB);
				}
			}
			sym->p.seg = seg->entry;
			//if( sym->p.seg->isabs ) /* jwlink: ensure p.seg is != NULL */
			if (sym->p.seg && sym->p.seg->isabs)
				DO_OR_EQ(sym_info, sym->info, |=, SYM_ABSOLUTE);
			TryDefVector(sym);
		}
		else
		{
	...
	/*****************************************************************************/
{
	...
	class1 = FindClass(sect, class_name, !use_16, FALSE, 0);
	info = 0;
	sdata = AllocSegData();
	...
	leader = FindALeader(sdata, class1, info);
	if (group_name != NULL)
	{
...
/********************************************************/
{
	...
	sym->p.seg = first;
	DO_OR_EQ(sym_info, sym->info, |=, SYM_DEFINED);
}
	...
	/*********************************************************************/
{
	...
	if (!(sym->info & SYM_DEFINED) || IS_SYM_IMPORTED(sym))
	{
		...
		ClearSymUnion(sym);
		SET_SYM_TYPE(sym_info, sym, symtype);
		DO_OR_EQ(sym_info, sym->info, |=, SYM_DEFINED);
		if (isfar)
		{
			DO_OR_EQ(sym_info, sym->info, |=, SYM_FAR_COMMUNAL);
		}
...
// handled properly.  no prob. under coff, but OMF makes it very hard...
{
	...
	symflags = (sym_info)(sym->info & SYM_CDAT_SEL_MASK);
...
/*****************************************************/
{
	...
	sym->addr.off = 0;
	sym->mod = CurrMod;
	SET_SYM_TYPE(sym_info, sym, SYM_COMDAT);
	sym->p.seg = sdata;
}
	...
	/******************************************************************/
{
	...
	else
	{
		...
		ClearSymUnion(sym);
		DO_OR_EQ(sym_info, sym->info, |=, select);
...
/* handle the lazy and weak extdef comments */
{
		...
		&& !IS_SYM_COMMUNAL(sym))
	{
		...
		else if (!(sym->info & SYM_OLDHAT) || IS_SYM_LINK_WEAK(sym))
		{
			if (isweak)
			{
				SET_SYM_TYPE(sym_info, sym, SYM_WEAK_REF);
			}
			else
			{
				SET_SYM_TYPE(sym_info, sym, SYM_LAZY_REF);
			}
			sym->e.def = def;
			if (LinkFlags & STRIP_CODE)
			{
	 ...
	 * to the dead code graph if necessary */
{
	...
	if (generate)
	{
		_ChkAlloc(symbol**, symlist, sizeof(symbol*) * count);
		liststart = symlist;
		*symlist = defsym;
		symlist++;
	...
	// change sym into a virtual function reference
{
	...
	DO_OR_EQ(sym_info, sym->info, |=, SYM_VF_REFS_DONE);
	if (symlist != NULL)
	{
		sym->e.vfdata = symlist;
		if (ispure)
		{
			SET_SYM_TYPE(sym_info, sym, SYM_PURE_REF);
		}
		else
		{
			SET_SYM_TYPE(sym_info, sym, SYM_VF_REF);
		}
	}
	else
	{                    // might still need this if eliminated
...
// we have an object file reference for sym
{
	...
	{
		DO_AND_EQ(sym_info, sym->info, &=, ~SYM_CHECKED);
...
/*******************************/
{
	...
	if (!HintFormat((exe_format)(MK_OS2 | MK_PE | MK_ELF | MK_NOVELL)))
	{
	...
	// handle the import coment record
{
	...
	if (!(sym->info & SYM_DEFINED))
	{
		...
		{
			SET_SYM_TYPE(sym_info, sym, SYM_IMPORTED);
			DO_OR_EQ(sym_info, sym->info, |=, SYM_DEFINED | SYM_DCE_REF);
...
/**********************************************/
{
	...
	sym = SymOp((sym_flags)(ST_CREATE | ST_REFERENCE), expname->name, expname->len);
	DO_OR_EQ(sym_info, sym->info, |=, SYM_EXPORTED);
	AddNameTable(expname->name, expname->len, TRUE, &FmtData.u.nov.exp.export1);
}
