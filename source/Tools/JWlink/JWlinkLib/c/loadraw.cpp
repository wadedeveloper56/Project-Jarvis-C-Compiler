...
/* returns TRUE if the file should be repositioned */
{
	...
	repos = FALSE;
	if (group->size != 0 || group->leaders->class1->flags & CLASS_COPY)
	{
		sect = group->section;
		CurrSect = sect;
		finfo = sect->outfile;
...
/***************************/
{
	...
	if (FmtData.type & (MK_PE | MK_QNX_FLAT | MK_OS2_FLAT | MK_ELF))
	{
		...
		{
			if (group->leaders->class1->flags & CLASS_COPY)
			{
				size = CalcGroupSize(group->leaders->class1->DupClass->segs->group);
			}
			else
			{
...
/******************************/
{
	...
	if (!linear)
	{
		if (nextAddr - (seg << seg_shift) + bufOfs > 0x10000L)
		{ // See if we need to output a segment record
	...
	{
		if (nextAddr - (seg << 16) + bufOfs > 0x10000L)
	...
	i = 0;
	while (bufOfs)
	{ // Do data bytes, leaving BufOfs 0 at end
...
/***************************************************/
{
	...
	if (!sdata->isuninit && !sdata->isdead && len > 0)
	{
		...
		{ // Must start new record if address not contiguous
			if (pieceAddr < nextAddr + bufOfs)
			{
				LnkMsg(ERR + MSG_FIXED_LOC_BEFORE_CUR_LOC, "a", (targ_addr*)_addr);
			}
			if (bufOfs)
			{              // If partial record in buffer, flush
				WriteHexLine();
			}
			nextAddr = pieceAddr;
		}
		...
		{                  // Now lob out records
			...
			if (piece > len)
			{         // Handle not enough to fill buffer
				piece = len;
			}
...
/**********************************************/
{
	if (!(((seg_leader*)seg)->class1->flags & CLASS_NOEMIT ||
		((seg_leader*)seg)->segflags & SEG_NOEMIT))
	{
		unsigned long   segaddr = *(unsigned long*)addr + GetLeaderDelta((seg_leader*)seg);
...
/************************************************/
{
	unsigned_32  segaddr = *(unsigned long*)addr + GetLeaderDelta((seg_leader*)seg);
...
/***************************/
{
	...
	if (FmtData.type & (MK_PE | MK_QNX_FLAT | MK_OS2_FLAT | MK_ELF))
	{
		...
		for (group = Groups; group != NULL; group = group->next_group)
		{
			...
			{
				wrkgrp = class1->DupClass->segs->group;
			}
			else
			{
				wrkgrp = group;
		...
		for (group = Groups; group != NULL; group = group->next_group)
		{
			...
			if (class1->flags & CLASS_COPY)
			{
				...
				DEBUG((DBG_LOADDOS, "group %a section %d to %l in %s",
					&group->grp_addr, sect->ovl_num, info.addr, finfo->fname));
				info.lastgrp = NULL;
				RingLookup(class1->DupClass->segs->group->leaders, WriteHexCopyGroups, &info);
			}
			else
			{
				if (group->size != 0)
