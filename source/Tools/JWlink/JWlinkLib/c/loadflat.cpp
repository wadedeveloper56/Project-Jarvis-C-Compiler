...
/* _write the object table and the object page map */
{
	...
	for (group = Groups; group != NULL; group = group->next_group)
	{
		...
		{ //if( group->u.miscflags & SEG_16_ALIAS ) {
	...
	if (start != NULL)
	{
		...
		while (place != NULL)
		{
			...
			for (; entries > 0; --entries, start = start->next)
			{
				...
				{
					// Forwarder entry
					dll_sym_info* dll = (dll_sym_info*)start->sym->p.import;
	...
	/**********************************************************************/
{
	...
	while (limit > 0)
	{
		...
		if (rptr != NULL)
		{
			...
			size += RelocSize((reloc_info*)*rptr++);
...
/********************************************************************/
{
	...
	if (sym->info & SYM_EXPORTED)
	{
		...
		dll->isordinal = TRUE;
		dll->m.modnum = NULL;
		dll->u.ordinal = ((entry_export*)sym->e.export1)->ordinal;
		*dllhandle = dll;
		return TRUE;
	}
	return FALSE;
