...
// information is provided.
{
	...
	for (currcl = sec->classlist; currcl != NULL; currcl = nextcl)
	{
		...
		{
			if (_stricmp(currcl->name, MatchClass->Name) == 0)
			{ // search order list for name match
				NewRing = &(MatchClass->Ring);   // if found save ptr to instance
				if (MatchClass->FixedAddr)
				{     // and copy any flags or address from it
					DO_OR_EQ(class_status, currcl->flags, |=, CLASS_FIXED);
					currcl->BaseAddr = MatchClass->Base;
					FmtData.base = 0;  // Otherwise PE will use default and blow up
				}
				if (MatchClass->NoEmit)
				{
					DO_OR_EQ(class_status, currcl->flags, |=, CLASS_NOEMIT);
				}
				break;
			}
		}
	...
	*owner = NULL;
	...
	for (MatchClass = sec->orderlist; MatchClass != NULL; MatchClass = MatchClass->NextClass)
	{
		if (MatchClass->Copy && MatchClass->Ring != NULL)
		{   // If this is a duplicate destination, find the source
			for (currcl = sec->classlist; currcl != NULL; currcl = currcl->next_class)
			{
				if (_stricmp(MatchClass->SrcName, currcl->name) == 0)
				{
					...
					DO_OR_EQ(class_status, MatchClass->Ring->flags, |=, CLASS_COPY);
					break;
				}
			}
		}
...
/****************************************************/
{
	...
	{
		DO_OR_EQ(class_status, seg->class1->flags, |=, CLASS_LXDATA_SEEN);
		return(TRUE);
	}
	return(FALSE);
}
...
// reorder segments based on COFF name grouping
{
	...
	for (currcl = Root->classlist; currcl != NULL; currcl = currcl->next_class)
	{
		foundgroup = FALSE;
		newlist = NULL;
		curr = (seg_leader*)RingPop(&currcl->segs);
		while (curr != NULL)
		{
			...
			{
				prev = NULL;
				comp = (seg_leader*)RingStep(newlist, NULL);
				foundmatch = FALSE;
				while (comp != NULL)
				{
					...
					prev = comp;
					comp = (seg_leader*)RingStep(newlist, comp);
				}
			}
			if (!added)
			{
 ...
 * segments (groups) are. */
{
	...
	for (class1 = Root->classlist; class1 != NULL; class1 = class1->next_class)
	{
		...
		{
			RingWalk(class1->segs, SetLeaderSeg);
		}
	}
}
 ...
 * DGROUP. So - this finds the start of the uninitialized portion of DGROUP */
{
	...
	for (class1 = Root->classlist; class1 != NULL; class1 = class1->next_class)
	{
		if (!(class1->flags & CLASS_DEBUG_INFO))
		{
			if (class1->flags & CLASS_LXDATA_SEEN)
			{
				setnext = TRUE;
			}
			else if (setnext)
			{
				FmtData.dgroupsplitseg = (seg_leader*)RingFirst(class1->segs);
				setnext = FALSE;
			}
		}
	}
	if (setnext)
	{             //last one was LXDATA or no segs.
		FmtData.dgroupsplitseg = NULL;
	}
}
...
{
	seg_leader* seg = (seg_leader*)_seg;
	grpaddrinfo* info = (grpaddrinfo*)_info;
...
// This is really only advantageous if uninitialized segments are at the end
{
	...
	{
		if (info->first_time)
		{ // First time, use seg_addr values
			info->grp_addr = seg_addr;
			info->end_addr = seg_addr + seg->size;
			info->first_time = FALSE;
		}
		else
		{  // If more segs found, use lowest start address and highest end address;
			if (info->grp_addr > seg_addr)
			{
				info->grp_addr = seg_addr;
			}
...
/************************************************/
{
	...
	if (info->lastgrp != seg->group)
	{   // Only interate new groups
 ...
 * not useful for OS/2 16-bit mode. */
{
	...
	for (; currgrp != NULL; currgrp = currgrp->next_group)
	{
		...
		if (class1->flags & CLASS_COPY)
		{
			...
			RingLookup(class1->DupClass->segs, FindCopyGroups, &info);
			...
			while ((class1 = class1->next_class) != NULL)
			{
				if (class1->flags & CLASS_FIXED)
				{
					save = class1->BaseAddr;     // If class is fixed, can stop
					ChkLocated(&save, TRUE);  //   after making sure address
					break;                      //   isn't already past here
				}
				if (!(class1->flags & CLASS_DEBUG_INFO))
				{ // skip Debug classes, they've already been done
					RingWalk(class1->segs, AllocSeg);
				}
			}
		}
		else
...
/* Allocate all classes in the list */
{
	...
	{
		DEBUG((DBG_OLD, "Allocating class %s", class1->name));
		if (class1->flags & CLASS_DEBUG_INFO)
		{
			...
			size = CurrSect->size;
			RingWalk(class1->segs, AllocSeg);
			DBIDefClass(class1, CurrSect->size - size);
			CurrSect->size = size;
			CurrLoc = save;
		}
		else
		{
			if (FmtData.type & (MK_PE | MK_QNX_FLAT | MK_OS2_FLAT | MK_ELF))
			{
				...
				if (class1->flags & CLASS_FIXED)
				{
					class1->segs->group->grp_addr.off = class1->BaseAddr.off;
			...
			{
				// segmented
				save = class1->BaseAddr;
				ChkLocated(&save, class1->flags & CLASS_FIXED);    // Process fixed locations if any
			}
			RingWalk(class1->segs, AllocSeg);
		}
	}
...
/*******************************/
{
	...
	if ((MapFlags & MAP_GLOBAL) && (NumMapSyms > 0))
	{
		...
		{
			_LnkAlloc(symbol**, symarray, NumMapSyms * sizeof(symbol*));
		}
		currsym = symarray;
		ok = symarray != NULL;
		for (sym = HeadSym; sym != NULL; sym = sym->link)
		{
			...
			{
				DO_AND_EQ(sym_info, sym->info, &=, ~SYM_MAP_GLOBAL);
				if (ok)
				{
					*currsym = sym;
					currsym++;
...
/**********************************************/
{
	symbol* pub = (symbol*)_pub;
	pubdefinfo* info = (pubdefinfo*)_info;
	segdata* seg;
	seg_leader* leader;
	offset      off;
	unsigned_16 frame;
	...
	if ((MapFlags & MAP_FLAG) && !SkipSymbol(pub))
	{
		...
		if (MapFlags & MAP_SORT)
		{
			...
			{
				NumMapSyms++;
				DO_OR_EQ(sym_info, pub->info, |=, SYM_MAP_GLOBAL);
			}
			else
			{
...
/* Process public definitions for an object file. */
{
	...
	{
		_ChkAlloc(symbol**, info.symarray, Ring2Count(CurrMod->publist) * sizeof(symbol*));
	}
	info.num = 0;
	info.first = TRUE;
	info.sect = sect;
...
/**********************************************/
{
	...
	for (class1 = Root->classlist; class1 != NULL; class1 = class1->next_class)
	{
		RingWalk(class1->segs, SetReadOnly);
	}
	start = flag_list;
...
/*********************************************************/
{
	...
	for (class1 = Root->classlist; class1 != NULL; class1 = class1->next_class)
	{
		...
		{
			RingLookup(class1->segs, SetClassFlag, &flags);
			return;
		}
	}
...
/***************************************************************/
{
	...
	for (class1 = Root->classlist; class1 != NULL; class1 = class1->next_class)
	{
		if (clflags == (class1->flags & CLASS_CODE))
		{
			RingLookup(class1->segs, SetClassFlag, &flags);
		}
	}
...
// this finds the floating point fixup symbols and marks them.
{
		...
		index++)
	{
		...
		if (sym != NULL)
		{
			SET_FFIX_VALUE(sym_info, sym, FloatNames[index].idx);
		}
	}
}
