...
/*******************************************/
{
	_PermAlloc(DWARFMODINFO*, mod->d.d, sizeof(dwarfmodinfo));
...
// calculate the amount of space needed to hold all of the line # info.
{
	...
	while (size > 0)
	{
		...
		else
		{
			...
			lineptr = (ln_off_pair*)((char*)lineptr + sizeof(ln_off_286));
...
/*************************************************/
{
	...
	DBILineWalk((lineinfo*)mod->lines, DwarfAddLines);
...
/**********************************************************/
{
	arange_tuple* tuple = (arange_tuple*)_tuple;
	...
	/*********************************************************************/
{
	...
	if (!(mod->modinfo & MOD_DBI_SEEN))
	{
		if (FmtData.type & MK_SEGMENTED)
		{
			...
			if (sdata->u.leader->class1->flags & CLASS_STACK)
			{
...
/*********************/
{
	...
	_ChkAlloc(Elf32_Shdr*, sect_header, shdr_size);
