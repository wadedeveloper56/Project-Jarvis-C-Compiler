...
// return TRUE if this is can be handled by ORL
{
	...
	if (type == ORL_ELF)
	{
		DO_OR_EQ(obj_format, ObjFormat, |=, FMT_ELF);
	}
	else if (type == ORL_COFF)
	{
		DO_OR_EQ(obj_format, ObjFormat, |=, FMT_COFF);
	}
	else
	{
		isOK = FALSE;
...
/***********************************************/
{
	...
	else if (machtype == ORL_MACHINE_TYPE_AMD64)
	{ /* jwlink */
		Set64BitMode();
	}
	else
...
/****************************************************/
{
	...
	for (; *name; )
	{
		...
		AddCommentLib(name, strlen(name), (lib_priority)(LIB_PRIORITY_MAX - 1));
		name = end;
	}
	return(ORL_OKAY);
}
...
/***********************************************/
{
	...
	else if ((FmtData.type & (MK_PE | MK_ELF)) && (sdata->isreadonly))
	{ /* jwlink */
...
// declare the "segment"
{
	...
	snode = (segnode*)AllocNodeIdx(SegNodes, segidx);
	snode->entry = sdata;
	snode->handle = sec;
	...
	{
		SeenDLLRecord();
		DO_OR_EQ(module_flags, CurrMod->modinfo, |=, MOD_IMPORT_LIB);
...
/***************************************************/
{
	...
	else
	{
		...
		return((segnode*)FindNode(SegNodes, idx));
	}
}
	...
	/***************************************************************************/
{
	if (type & ORL_SYM_TYPE_UNDEFINED)
	{
		if (namelen > sizeof(CoffImportRefName) - 1)
		{
			...
			{
				_ChkAlloc(char*, ImpModName, namelen + 5);
...
/******************************************************************************/
{
	...
	{
		sym_type = (sym_info)((select - 1) << SYM_CDAT_SEL_SHIFT);
	}
...
/******************************************************/
{
	...
	if (type & ORL_SYM_TYPE_FILE)
	{
		...
		{
			DO_OR_EQ(module_flags, CurrMod->modinfo, |=, MOD_GOT_NAME);
			...
			name != NULL))
	{
		...
		newnode = (extnode*)AllocNode(ExtNodes);
		...
		{
			//symop |= ST_STATIC;
			DO_OR_EQ(sym_flags, symop, |=, ST_STATIC | ST_DUPLICATE);
		}
		if (type & ORL_SYM_TYPE_UNDEFINED && binding != ORL_SYM_BINDING_ALIAS)
		{
			DO_OR_EQ(sym_flags, symop, |=, ST_REFERENCE);
		}
		else
		{
			DO_OR_EQ(sym_flags, symop, |=, ST_NOALIAS);
		}
		...
		else if (type & ORL_SYM_TYPE_UNDEFINED)
		{
			...
			switch (binding)
			{
				...
				case ORL_SYM_BINDING_LAZY:
					...
					{
						assocsym = SymOp((sym_flags)(ST_CREATE | ST_REFERENCE), name, namelen);
...
/*******************************************/
{
	...
	switch (reloc->type)
	{
		...
		case ORL_RELOC_TYPE_ABSOLUTE:
			type = (fix_type)(FIX_OFFSET_32 | FIX_ABS);
			break;
		case ORL_RELOC_TYPE_WORD_16:
			type = (fix_type)FIX_OFFSET_16;
			break;
		case ORL_RELOC_TYPE_WORD_26:
			type = (fix_type)(FIX_OFFSET_26 | FIX_SHIFT);
			break;
		case ORL_RELOC_TYPE_TOCREL_14:  // relative ref to 14-bit offset from TOC base.
			type = (fix_type)FIX_SHIFT;           // NOTE fall through
		...
		case ORL_RELOC_TYPE_GOT_16:     // relative ref to 16-bit offset from TOC base.
			type = (fix_type)(type | FIX_TOC | FIX_OFFSET_16);
			istoc = TRUE;
			break;
		...
		case ORL_RELOC_TYPE_TOCVREL_16:  // relative ref to 16-bit offset from TOC base.
			DO_OR_EQ(fix_type, type, |=, FIX_TOCV | FIX_OFFSET_16);
			break;
		case ORL_RELOC_TYPE_IFGLUE:
			type = (fix_type)(FIX_IFGLUE | FIX_OFFSET_32);
			break;
		case ORL_RELOC_TYPE_IMGLUE:
			skip = TRUE;                 // NYI: do we need this?
			break;
		case ORL_RELOC_TYPE_JUMP:
			type = (fix_type)(FIX_OFFSET_32 | FIX_REL);
			break;
		case ORL_RELOC_TYPE_REL_21_SH:
			type = (fix_type)(FIX_OFFSET_21 | FIX_REL | FIX_SHIFT);
			break;
		case ORL_RELOC_TYPE_REL_24:
			type = (fix_type)(FIX_OFFSET_24 | FIX_REL);
			break;
		...
		case ORL_RELOC_TYPE_REL_32_NOADJ:
			type = (fix_type)(FIX_OFFSET_32 | FIX_REL | FIX_NOADJ);
		...
		case ORL_RELOC_TYPE_GOT_32:     // relative ref to 32-bit offset from TOC base.
			type = (fix_type)(FIX_OFFSET_32 | FIX_TOC);
			break;
		...
		case ORL_RELOC_TYPE_HALF_LO:
			if (SavedReloc.type == ORL_RELOC_TYPE_NONE)
			{    // we recursed
				type = (fix_type)(FIX_OFFSET_16 | FIX_SIGNED);
			}
			else
			{
				...
				DoReloc(reloc);
				reloc = &SavedReloc;
				type = (fix_type)FIX_HIGH_OFFSET_16;
			}
			break;
		case ORL_RELOC_TYPE_WORD_64: /* jwlink */
			type = (fix_type)FIX_OFFSET_64;
...
// do pass 1 for an object file handled by ORL.
{
	...
	if (CheckFlags(filehdl))
	{
		...
		{
			DO_OR_EQ(module_flags, CurrMod->modinfo, |=, MOD_FLATTEN_DBI);
		}
		DO_OR_EQ(module_flags, CurrMod->modinfo, |=, MOD_NEED_PASS_2);
