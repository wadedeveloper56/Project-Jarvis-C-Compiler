...
/*******************************************/
{
	...
	{
		if (class1->flags & CLASS_STACK)
		{
			return((seg_leader*)RingFirst(class1->segs));
		}
	}
	return(NULL);
}
			...
			start addresses */
{
	...
	{
		chk = (symbol*)Ring2Lookup(StartInfo.mod->publist, CompSymPtr, sym);
	}
	if (chk == NULL)
	{
...
static void DefBSSStartSize(char* name, class_entry* class1)
/*************************************************************/
/* set the value of a start symbol, and see if it has been defined */
{
	...
	if (sym->addr.seg == UNDEFINED)
	{
		...
		seg = (seg_leader*)RingFirst(class1->segs);
...
static void DefBSSEndSize(char* name, class_entry* class1)
/***********************************************************/
/* set the value of an end symbol, and see if it has been defined */
{
	...
	if (sym->addr.seg == UNDEFINED)
	{
		...
		seg = (seg_leader*)RingLast(class1->segs);
...
/* Find size of BSS segment, and set the special symbols */
{
	...
	class1 = LocateBSSClass();
	if (class1 != NULL)
	{
		/* set start of BSS class */
		DefBSSStartSize(BSSStartSym, class1);
		DefBSSStartSize(BSS_StartSym, class1);
		DefBSSEndSize(BSSEndSym, class1);
		DefBSSEndSize(BSS_EndSym, class1);
	}
}
...
/***********************************/
{
	...
	else
	{
		...
		DO_OR_EQ(sym_info, StartInfo.targ.sym->info, |=, SYM_DCE_REF);
...
/*****************************/
{
	...
	{
		_ChkAlloc(char*, ImpLib.buffer, IMPLIB_BUFSIZE);
		...
		_ChkAlloc(char*, ImpLib.module_name, ImpLib.module_name_len + 1);
...
/**************************/
{
	...
	_ChkAlloc(char*, atfname, namelen + 1);  // +1 for the @
	*atfname = '@';
	...
	else if ((FmtData.type & MK_PE) && FmtData.u.pe.win64)
	{   /* jwlink: AMD64 import lib support */
		libtype = "-i6";
	}
	else
	{
...
/**************************************************************************/
{
	...
	buff = (char*)malloc(intlen + otherlen + ImpLib.module_name_len + 13);
	buff[0] = '+';
	buff[1] = '+';
	buff[2] = '\'';
	currpos = buff + 3;
...
/***************************************************/
{
	...
	grpwriteinfo* info = (grpwriteinfo*)_info;
...
/**************************************/
{
	...
	DoWriteLeader((seg_leader*)seg, &info);
}
...
/**************************************************/
{
	...
	if (!(seg->class1->flags & CLASS_NOEMIT ||
...
// As copies, they need to be output, so ignore their MOEMIT flag here
{
	...
	info->seg_start = info->grp_start + GetLeaderDelta((seg_leader*)seg);
	DoWriteLeader((seg_leader*)seg, info);
	return(FALSE);
}
...
/*********************************************************/
{
	...
	if (class1->flags & CLASS_COPY)
	{
		...
		RingLookup(class1->DupClass->segs->group->leaders, WriteCopyGroups, &info);
	}
	else
	{
...
/**********************************************/
{
	...
	_ChkAlloc(char*, outfile->buffer, BUFF_BLOCK_SIZE);
}
