...
/*********************************/
{
	...
	{
		_Pass1Alloc(edgelist*, edge, sizeof(edgelist));
	}
...
/* remove any edges from the list which don't point to symbols */
{
	...
	while (list != NULL)
	{
		...
		if (list->issym)
		{
			list->next = (edgelist*)sym->p.edges;
			sym->p.edges = list;
		}
		else
		{
...
/* symbol referenced from data, so make sure it is included */
{
	...
	DO_OR_EQ(sym_info, sym->info, |=, SYM_DCE_REF);
}
...
/* reference from a segment to a symbol */
{
	...
	{    /* symbol is undefined */
		...
		else
		{
			edge = AllocEdge();
			edge->u.seg = seg;
			edge->next = (edgelist*)sym->p.edges;
			edge->issym = FALSE;
...
/* make a reference from one symbol to another */
{
	...
	else
	{
		...
		edge->u.sym = targsym;
		edge->issym = TRUE;
		edge->next = (edgelist*)srcsym->p.edges;
		srcsym->p.edges = edge;
	}
}
...
/* make a reference from symbol to segment */
{
	...
	if (srcsym->info & SYM_DEFINED)
	{
		edge->next = (edgelist*)targsdata->a.refs;
...
*/
{
	...
	while (list != NULL)
	{
		...
		else
		{
			...
			if (trg->isrefd)
			{ /* seg is already refd, so don't need the edge*/
				FreeEdge(list);
			}
			else
			{
				...
				else
				{
					...
					list->next = (edgelist*)src->a.refs;   // & add it to the ref list
					src->a.refs = list;
				}
			}
		}
...
*/
{
	...
	while (list != NULL)
	{
		...
		if (list->reverse_dir == 0)
		{
			if (list->issym)
			{
				DO_OR_EQ(sym_info, list->u.sym->info, |=, SYM_DCE_REF);
			}
			else
			{
				RefSeg(list->u.seg);
...
{
	FreeEdgeList((edgelist*)sym->p.edges);
	sym->p.edges = NULL;
}
