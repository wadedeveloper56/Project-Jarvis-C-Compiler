...
/**************************************************/
{
	...
	_PermAlloc(snamelist*, node, sizeof(snamelist) + len);
	node->len = len;
...
/***********************************/
{
	...
	_PermAlloc(void*, sect->dbg_info, sizeof(debug_info));
...
/******************************************/
{
	if (CurrSect->dbg_info == NULL)
		return;
	_PermAlloc(odbimodinfo*, mod->d.o, sizeof(odbimodinfo));
...
/*********************************************************************/
{
	...
	if (major != Master.obj_major_ver)
	{
		...
		DO_AND_EQ(module_flags, CurrMod->modinfo, &=, ~(DBI_TYPE | DBI_LOCAL));
	}
...
/*********************************************************/
{
	...
	dinfo = (debug_info*)CurrSect->dbg_info;
	if (dinfo == NULL)
		return;
...
/************************************************/
{
	...
	{
		DBILineWalk((lineinfo*)obj->lines, ODBIAddLines);
	}
...
/************************************************/
{
	((seg_leader*)leader)->group = (group_entry*)group;
	return(FALSE);
}

static void AllocDBIClasses(class_entry* class1)
/***********************************************/
/* Allocate all classes in the list */
{
	...
	for (; class1 != NULL; class1 = class1->next_class)
	{
		if (class1->flags & CLASS_DEBUG_INFO)
		{
			...
			RingLookup(class1->segs, AllocASeg, group);
		}
	}
}
...
/********************************************************/
{
	segheader   seghdr;
	seg_leader* seg;
	debug_info* dptr = (debug_info*)_dinfo;
...
/********************************************/
{
	...
	if (CurrSect == NonSect)
	{
		dptr = (debug_info*)Root->dbg_info;
	}
	else
	{
		dptr = (debug_info*)CurrSect->dbg_info;
	}
...
/***************************************************/
{
	...
	segdata** firstseg = (segdata**)_firstseg;
...
/*********************************/
{
	...
	*/
	firstseg = (segdata*)Ring2Step(CurrMod->segs, NULL);
	...
	if (info->size & LINE_IS_32BIT)
	{
		while (size > 0)
		{
			...
			pair = (ln_off_pair*)((char*)pair + sizeof(ln_off_386));
	...
	else
	{
		while (size > 0)
		{
			...
			prevoff = temp;
			pair = (ln_off_pair*)((char*)pair + sizeof(ln_off_286));
