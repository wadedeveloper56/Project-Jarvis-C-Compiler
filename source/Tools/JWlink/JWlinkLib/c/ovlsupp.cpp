...
/******************************/
{
	...
	while (vec != NULL)
	{
		...
		WriteMap("%a section %d : %S", &addr,
			sym->p.seg->u.leader->class1->section->ovl_num, sym);
		vec = vec->next;
	}
}
...
/* Allocate parallel overlay sections */
{
	...
	if (first_sect == NonSect)
	{
		if (FmtData.u.dos.dynamic)
		{
			...
			area_specified = TRUE;
			if (AreaSize == 0xFFFF)
			{   // default is twice the memory size
				AreaSize = ovl_size * 2;
...
/**************************************/
{
	...
	{
		ovl_num = sdata->u.leader->class1->section->ovl_num;
	}
	if (!sdata->iscode || (ovl_num == 0))
	{      // not code or in root
		sym->u.d.ovlstate |= (OVL_FORCE | OVL_NO_VECTOR);
	}
	else
...
/* allocate an overlay vector for a symbol */
{
	...
	sym->u.d.ovlstate |= OVL_FORCE;
	_PermAlloc(vecnode*, vec, sizeof(vecnode));
	vec->entry = sym;
...
/**************************************/
{
	...
	else
	{
		...
		ovl_num = sym->p.seg->u.leader->class1->section->ovl_num;
...
/********************************************************/
{
	...
	if ((sym->u.d.ovlstate & OVL_VEC_MASK) != OVL_MAKE_VECTOR)
		return;
	if (IsAncestor(sym->p.seg->u.leader->class1->section->ovl_num, CurrSect))
	{
		return;
	}
...
/* check if among overlay classes, and return TRUE if it is code. */
{
	...
	if (cnamelist == NULL)
	{   /* check for anything ending in code */
		*isovlclass = retval;
		return(retval);
	}
	else
...
/*****************************************/
{
	...
	template1.call_op = 0xe8;
	template1.jmp_op = 0xe9;     // near jmp
	vecnum = 1;
	for (vec = OvlVectors; vec != NULL; vec = vec->next)
	{
		...
		_HostU16toTarg(diff, template1.ldr_addr);
		loadsym = vec->entry;
		_HostU16toTarg(loadsym->p.seg->u.leader->class1->section->ovl_num,
			template1.sec_num);
		...
		_HostU16toTarg(diff, template1.target);
		PutOvlInfo(vect_off, &template1, sizeof(svector));
...
/****************************************/
{
	...
	vect_off = OvlvecAddr.off;
	/* fill in overlay vector template */
	template1.u.v.call_op = 0xe8;
	template1.jmp_op = 0xea;     // far jmp
	...
	for (vec = OvlVectors; vec != NULL; vec = vec->next)
	{
		...
		_HostU16toTarg(diff, template1.u.v.ldr_addr);
		loadsym = vec->entry;
		_HostU16toTarg(loadsym->p.seg->u.leader->class1->section->ovl_num,
			template1.u.v.sec_num);
		_HostU16toTarg(loadsym->addr.off, template1.target.off);
		if (FmtData.u.dos.dynamic)
		{
			_HostU16toTarg(loadsym->addr.seg
				- loadsym->p.seg->u.leader->group->grp_addr.seg,
				template1.target.seg);
		}
		else
		{
			_HostU16toTarg(loadsym->addr.seg, template1.target.seg);
			addr.off = vect_off + offsetof(lvector, target.seg);
			WriteReloc(OvlGroup, addr.off, &addr, sizeof(dos_addr));
		}
		PutOvlInfo(vect_off, &template1, sizeof(lvector));
...
/**************************/
{
	...
	OvlSeg = InitLeader("");
	OvlSeg->class1 = FindClass(Root, OvlMgrClass, 0, 1, 0);
...
/* generate overlay table */
{
	...
	/*
		Generate prolog :
	*/
	template1.major = OVL_MAJOR_VERSION;
	template1.minor = OVL_MINOR_VERSION;
	/*
		output start address for program
		reloc for this was emitted by EmitOvlVectors
	*/
	_HostU16toTarg(Stash.off, template1.start.off);
	_HostU16toTarg(Stash.seg, template1.start.seg);
	/*
		this should give us the paragraph of the load module start
		reloc for this was emitted by EmitOvlVectors
	*/
	_HostU16toTarg(0, template1.delta);
	_HostU16toTarg(AreaSize, template1.ovl_size);

	PutOvlInfo(off, &template1, sizeof(template1));
	/*
		Generate entries :
	*/
	off += sizeof(template1);
